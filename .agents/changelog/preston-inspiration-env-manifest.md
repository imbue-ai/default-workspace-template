...
An template publishes `template.md`, `template.toml`, and
...
cargo crates, and `env.d` units the code needs. Adopting an template
...
When publishing an template, the agent now renders the generated README into
...
Adopting an template installs what it declares with ordinary commands rather
