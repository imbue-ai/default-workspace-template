...
indistinguishable from the live interface. This server wraps that inner service
in a chrome page that frames it on all four sides: an accent border + a header
label marking the tab as a *preview of a proposed change*, with the inner
service held in a bordered "stage" iframe inside that frame.
...
The wrapper itself is registered as its own service, so it is served at its own
origin too. Every service origin is a sibling hostname of the wrapper's own
(locally ``<name>.<workspace-host>``; on shares ``<name>--<host>--<user>.<domain>``),
and the workspace host cannot be known server-side -- so the inner iframe's
``src`` is derived in the page's JavaScript from ``location.host`` by swapping
the leading service coordinate for the inner service's name.
...
<body>
  ...
  <script>
    ...
    var innerHost;
    if (firstLabel.indexOf("--") !== -1) {{
      innerHost = previewService + host.slice(host.indexOf("--"));
    }} else {{
      innerHost = previewService + host.slice(host.indexOf("."));
    }}
