...
- **Preview (surface to the user).** Add ``--service-name`` to also register the
  ...
  letters/digits with single hyphens, no underscores (e.g. ``preview-1``, not
  ``preview_1``), and not starting with ``agent-``. ``down`` kills both servers
  and deregisters both services.
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    up_parser.add_argument(
        ...
        "not starting with 'agent-'). Omit for a bare instance reached directly "
        "on its port.",
    )
    up_parser.add_argument(
