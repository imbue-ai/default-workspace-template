...
- **A measured cost/time estimate, when the flow has a metered `[ai-script]`
  step** (a real model call -- especially one fanned out across N items or one
  that uses tools / web search). Do not guess from token counts: run one real
  unit, capture its actual cost and wall-clock, and extrapolate to a full run
  (`N x per-unit` + expected retries + tool/search fees). This lets the user
  approve the run cost at the outline gate rather than discovering it after the
  pipeline is built. The measurement technique is in
  `.agents/skills/use-ai-integration/references/hardening-scripted-calls.md`.
