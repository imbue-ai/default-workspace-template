...
`http://<name>.<workspace-host>/` (and, on shares,
`https://<name>--<host>--<user>.<domain>/`) straight to the backend URL
you registered. Nothing proxies, rewrites, or prefixes your app's
traffic, so root-absolute URLs (`href="/api"`), `new WebSocket("/ws")`,
`Set-Cookie: Path=/`, and service workers all work exactly as written.
...
values (`/login` is fine now -- it stays on your origin), never the
