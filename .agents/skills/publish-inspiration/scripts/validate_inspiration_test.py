...
_SCRIPT = Path(__file__).resolve().parent / "validate_inspiration.py"
_spec = importlib.util.spec_from_file_location("validate_inspiration", _SCRIPT)
assert _spec is not None and _spec.loader is not None
validate_inspiration = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(validate_inspiration)
...
def test_a_shallow_snapshot_dir_still_finds_the_sibling_schema() -> None:
    ...
    `build_inspiration.sh` snapshots this script and the schema module into a
    ...
    """
    shallow = Path("/tmp/tmp.ABC123/validate_inspiration.py")

    candidates = validate_inspiration._schema_module_candidates(shallow)

    assert candidates[0] == Path("/tmp/tmp.ABC123/inspiration_manifest.py")
...
def test_the_sibling_snapshot_is_preferred_over_any_repo_copy() -> None:
    ...
    deep = Path("/a/b/c/d/e/scripts/validate_inspiration.py")

    candidates = validate_inspiration._schema_module_candidates(deep)

    assert candidates[0] == Path("/a/b/c/d/e/scripts/inspiration_manifest.py")
...
def test_every_ancestor_is_offered_as_a_repo_root() -> None:
    ...
    assert Path("/a/b/c/d/e") / validate_inspiration._IN_REPO_SCHEMA_PATH in candidates
    assert Path("/a") / validate_inspiration._IN_REPO_SCHEMA_PATH in candidates
