...
def _template(tmp_path: Path) -> Path:
    ...
    _commit(template, f"Improve the readme (supersedes {proposal[:10]})", {"README.md": "two\n"})
...
def _rewritten_upstream(tmp_path: Path, template: Path) -> Path:
    ...
    subprocess.run(
        [*FILTER_REPO_COMMAND, *FILTER_REPO_ARGS], cwd=upstream, capture_output=True, check=True
    )
...
def _workspace(tmp_path: Path, template: Path, upstream: Path, *, depth: int | None = None) -> Path:
    """A workspace created from ``minds-v1`` before the rewrite, with its own commits."""
    workspace = tmp_path / "workspace"
    depth_args = ["--depth", str(depth)] if depth else []
    _git(tmp_path, "clone", "-q", *depth_args, "--branch", "minds-v1", template.as_uri(), str(workspace))
    _git(workspace, "checkout", "-q", "-B", "main")
    _commit(workspace, "Add my notes", {"notes.md": "mine\n"})
    _commit(workspace, "Tweak the readme and vendored mngr", {"README.md": "mine\n", VENDORED_FILE: "mine\n"})
...
def _rewrite(workspace: Path, capsys) -> dict:
    assert update_self.main(["rewrite-history", "--ref", "minds-v2", "--repo-root", str(workspace)]) == 0
...
def test_rewrite_history_gives_an_old_workspace_the_rewritten_fork_point(tmp_path, capsys) -> None:
    template = _template(tmp_path)
    upstream = _rewritten_upstream(tmp_path, template)
    workspace = _workspace(tmp_path, template, upstream)
    assert subprocess.run(["git", "merge-base", "HEAD", "minds-v2"], cwd=workspace).returncode == 1
    ...
    assert _git(workspace, "rev-parse", "minds-v1") == _git(upstream, "rev-parse", "minds-v1")
    assert _git(workspace, "log", "--format=%s", "-2").splitlines() == [
        "Tweak the readme and vendored mngr",
        "Add my notes",
    ]
    assert _git(workspace, "show", "--name-only", "--format=", "HEAD").splitlines() == ["README.md"]
    reachable = _git(workspace, "rev-list", "--objects", "--branches", "--tags").splitlines()
    assert not [line for line in reachable if " system/vendor" in line]
    assert _git(workspace, "status", "--porcelain") == ""
    assert (workspace / VENDORED_FILE).read_text() == "mine\n"
    assert not list((workspace / "data/.tasks/update-self/history-rewrite").glob("*.json"))
...
def test_rewrite_history_makes_the_next_tag_fetch_clean(tmp_path, capsys) -> None:
    ...
    fetch = subprocess.run(["git", "fetch", "upstream", "--tags"], cwd=workspace, capture_output=True, text=True)
...
def test_rewrite_history_is_a_no_op_once_history_is_shared(tmp_path, capsys) -> None:
    ...
    assert result == {"rewritten": False, "fork_point": _rewritten_v1(upstream), "moved_refs": 0}
    assert _git(workspace, "rev-parse", "HEAD") == head


def test_rewrite_history_carries_a_shallow_workspace_onto_the_rewritten_release(tmp_path, capsys) -> None:
    template = _template(tmp_path)
    upstream = _rewritten_upstream(tmp_path, template)
...
def test_rewrite_history_refuses_uncommitted_changes_and_changes_nothing(tmp_path, capsys) -> None:
    template = _template(tmp_path)
    upstream = _rewritten_upstream(tmp_path, template)
    workspace = _workspace(tmp_path, template, upstream)
    ...
    assert update_self.main(["rewrite-history", "--ref", "minds-v2", "--repo-root", str(workspace)]) == 1

    assert "uncommitted changes" in capsys.readouterr().err
    assert _git(workspace, "rev-parse", "HEAD") == head


def test_rewrite_history_refuses_a_workspace_the_rewrite_cannot_connect(tmp_path, capsys) -> None:
    template = _template(tmp_path)
    upstream = _rewritten_upstream(tmp_path, template)
    ...
    _git(stranger, "fetch", "-q", str(upstream), "refs/tags/minds-v2:refs/tags/minds-v2")
    head = _git(stranger, "rev-parse", "HEAD")

    assert update_self.main(["rewrite-history", "--ref", "minds-v2", "--repo-root", str(stranger)]) == 1
...
def test_rewrite_history_finishes_a_run_interrupted_after_moving_refs(tmp_path, capsys) -> None:
    template = _template(tmp_path)
    upstream = _rewritten_upstream(tmp_path, template)
    workspace = _workspace(tmp_path, template, upstream)
        ...
        json.dumps(
            {
                ...
                "worktrees": [{"path": str(workspace), "old": old_head, "new": new_head}],
            }
        )
    )
    assert _git(workspace, "status", "--porcelain") != ""
