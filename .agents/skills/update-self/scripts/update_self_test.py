...
def _supervisor_status(pid: int) -> _Result:
    return _Result(stdout=f"system_interface   RUNNING   pid {pid}, uptime 0:00:05\n")


def _apply_runner(name_status: str, repo_root: Path) -> _RecordingRunner:
    runner = _RecordingRunner(repo_root=repo_root)
    # A live service that has settled: the same pid on every status call.
...
def test_preflight_boot_follows_the_live_observer_and_uses_the_strict_health_path(
    ...
    """The pre-flight boots beside the live service, which holds the observe lock.

    An observer of its own would lose that lock and die, leaving the throwaway
    instance with a frozen agent view -- and ``/api/agents`` would pass it anyway,
    since it never looks at the lifecycle stream. So the boot follows the live
    observer and is judged on the strict health path; the live service keeps the
    looser probe, where a spurious rollback is the costlier mistake.
    """
    runner = _apply_runner(_BACKEND_DIFF, apply_repo)
    spawner = _FakeSpawner()
    http = _FakeHttp(_all_healthy)
    ...
    assert spawner.envs, "the pre-flight boot never spawned"
    assert all(
        env[update_probes.PREFLIGHT_AGENT_EVENTS_MODE_ENV]
        == update_probes.FOLLOW_AGENT_EVENTS_MODE
        for env in spawner.envs
    )
    preflight_urls = [url for url in http.get_urls if not _is_live(url)]
    assert preflight_urls and all(
        url.endswith(update_probes.STRICT_HEALTH_PATH) for url in preflight_urls
    )
    live_urls = [url for url in http.get_urls if _is_live(url)]
    assert live_urls and all(
        url.endswith(update_probes.HEALTH_PATH) for url in live_urls
    )
