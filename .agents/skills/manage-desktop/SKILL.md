...
| **window** | One page of one app on one desktop: the app, the path under the app's origin the page is at (`chat` at `/?chat=<id>`, `terminal` at `/?session=<name>`, `files` at `/data/notes/`), and the title the page last reported. Named by an id, `win-<hex>`. |
...
| **launch path** | A path an app declares for opening a new page of itself (`chat` and `terminal` declare `new` at `/new`; `files` declares `new` at `/data/` with a `path` parameter). An app that declares none offers `open` at `/`. |
...
| Open a specific page of an app | `python3 system/scripts/layout.py open files --path /data/notes/` |
...
/data/notes/` (dufs serves the workspace at `/`, so `/data/notes/` is `data/notes`; the `path`
launch parameter, `open files --param path=/data/notes/`, lands in the same folder
but as a window at `/data/?path=/data/notes/`, and a window is focused only when its
