...
into `/code`. This mirrors `use-inspiration` §1 and how `update-self` validates an
...
- **CRITICAL -- preserve this mind's own adaptations.** Merge conflicts mark
  ...
  what conflicts in plain language (§3), and only then redo the merge in `/code`
...
unresolved) and the boot check passes, fast-forward `/code` onto the exact commit
