...
kept, following the live observer's event stream rather than starting a second
observer (``SYSTEM_INTERFACE_AGENT_EVENTS_MODE=FOLLOW``), probe ``/api/health``
-- which stays red unless that stream is really feeding the preview -- and
...
Environment:
    MNGR_AGENT_ID  Dropped for the preview boot so it cannot clobber the live
                   layout.
    SYSTEM_INTERFACE_AGENT_EVENTS_MODE  Set to FOLLOW for the preview boot, so it
                   reads the live observer's event stream instead of fighting it
                   for the single-writer observe lock.
...
# system interface already holds that lock -- so a preview that tried to run its
# own observer would have it die seconds into boot, leaving the preview's agent
# list and chat panels frozen at boot state forever while terminals and
# everything else kept working. FOLLOW makes it read the live observer's event
# stream instead, which is all a read-only second instance ever needed. This is
# the env spelling of the server's ``Config.system_interface_agent_events_mode``.
PREVIEW_AGENT_EVENTS_MODE_ENV = "SYSTEM_INTERFACE_AGENT_EVENTS_MODE"
FOLLOW_AGENT_EVENTS_MODE = "FOLLOW"
# ``/api/health`` is the strict gate, handed to the shared preview script as its
# ``--health-path``. It asserts both that a fresh mngr discovery works -- the
# plugin/config path a missing backend dependency or a broken plugin-config
# parse would take down -- and that the preview's agent lifecycle event stream
# is actually live. That second half is why ``/api/agents`` is not enough: it
# runs its own discovery rather than reading the cache the lifecycle stream
# feeds, so it answers 200 on an instance whose agent view is dead. A preview
# that came up looking healthy and showed "No conversation data" for every agent
# created after it booted is exactly the gap this closes.
...
def preview(slug: str, work_dir: str, repo_root: Path, *, runner: Runner) -> int:
    ...
    keeping discovery, so the real conversations still render; run in FOLLOW
    mode so the preview reads the live observer's agent lifecycle stream instead
    of trying to start a second observer it cannot get the lock for; probe
    ``/api/health``, which stays red unless that stream really is feeding the
    preview; register the inner app and the labeled wrapper frame.
