...
#   runtime/jobs/<job-id>/last_attempt   epoch when a run last STARTED
#   runtime/jobs/<job-id>/last_success   epoch when a run last COMPLETED (exit 0)
#   runtime/jobs/<job-id>/failures       consecutive failed attempts
#
# Semantics:
...
#
# Test hooks (used by scripts/run_job_test.py): MINDS_JOB_STATE_DIR overrides
...
STATE_DIR="${MINDS_JOB_STATE_DIR:-$ROOT/runtime/jobs}/$JOB_ID"
mkdir -p "$STATE_DIR"
