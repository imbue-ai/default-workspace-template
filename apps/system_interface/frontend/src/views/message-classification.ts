...
export function isHiddenUserMessage(content: string): boolean {
  ...
  //
  // Note: the persistent Caretaker's "/caretaker" run trigger and the "/clear"
  // mngr sends before each run are deliberately NOT hidden -- the chat shows what
  // actually happened, rather than hiding the commands behind the scenes.
  const trimmed = content.trim();
  if (trimmed === "/welcome") {
    return true;
  }
  // Claude Code injects a local-command caveat ("<local-command-caveat>Caveat: The
  // messages below were generated by the user while running local commands. DO NOT
  // respond ...") after a local command such as /clear. It is addressed to the
  // agent, not the user, and is not a real user turn, so hide it. (The /clear and
  // /caretaker commands themselves are shown.)
