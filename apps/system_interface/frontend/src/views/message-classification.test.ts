...
describe("isHiddenUserMessage", () => {
  ...
  it("hides the /clear command and the local-command caveat that opens a cleared chat", () => {
    // mngr sends "/clear" before re-triggering the Caretaker; Claude Code then
    // prefixes the cleared session with a local-command caveat. Neither should
    // render, so the cleared chat starts at the Caretaker's own message.
    expect(isHiddenUserMessage("/clear")).toBe(true);
    expect(
      isHiddenUserMessage(
