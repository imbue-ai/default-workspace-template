 ...
 *
 * Callers pass the held events plus the step enrichment scoped to this
 * conversation; `agentIsIdle` settles the frontier spinner on the tail turn.
 * The pre-login auth-error prefix is hidden here (a no-op for a subagent, which
 * never has one) so the two views stay byte-identical.
 */

import m from "mithril";
import type { TranscriptEvent, StepEnrichment, ToolResultEvent } from "../models/Response";
import {
  renderUserMessage,
 ...
 * sections joined with the (already scoped) step enrichment, then flatten into
 * top-level rows. The structure -- which steps exist, their order, grouping --
 * comes purely from the transcript walk; tk enrichment is a side-table joined by
 * id.
 */
