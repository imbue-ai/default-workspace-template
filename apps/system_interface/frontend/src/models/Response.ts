...
// Step enrichment for subagent conversations, keyed by agentId + session id, so a
// subagent's steps never collide with (or leak into) the main view's table. The
// main view's enrichment lives in its TranscriptStore (so it rides the store's
// render-version commits); only subagent scopes use this side table. Replaced
// wholesale on each snapshot (GET events + the `step_enrichment` SSE message),
// never merged.
const enrichmentByScope: Record<string, Map<string, StepEnrichment>> = {};

/** Storage key for a subagent enrichment scope: agentId + its session id. */
...
/** Enrichment for a conversation. The main view (no `sessionId`) reads from its
 *  TranscriptStore; a subagent (`sessionId` given) reads its own scoped table. */
...
/** Replace a conversation's enrichment table from a snapshot. The main view (no
 *  `sessionId`) writes to its TranscriptStore; a subagent (`sessionId` given)
 *  writes its own scoped table. Does not redraw -- callers in a fetch/redraw flow
 *  already trigger one; the SSE path redraws explicitly. */
