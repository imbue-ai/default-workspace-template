...
def _default_read_assistant_transcript(agent: AgentInfo) -> str | None:
    ...
    (via the harness-appropriate `TranscriptWatcher`) and joins the
    `assistant_message` text. Only assistant turns are included: the
    `/welcome` skill expansion is a *user* message that also contains the
    welcome text verbatim, so including user turns would always look like a
    delivered welcome. Returns None when the transcript cannot be read so the
    caller treats the welcome as not-yet-delivered.
    """
    watcher = build_transcript_watcher(agent, on_events=lambda _agent_id, _events: None)
