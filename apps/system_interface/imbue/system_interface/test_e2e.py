...
def e2e_server(tmp_path: Path) -> Generator[tuple[str, list[AgentInfo], Path], None, None]:
    ...
    send_patcher = patch("imbue.system_interface.server.send_message_and_get_error", return_value=None)
    send_patcher.start()
...
def test_tool_calls_render_as_collapsible(tmp_path: Path, page: Page) -> None:
    ...
    with (
        ...
        patch("imbue.system_interface.server.send_message_and_get_error", return_value=None),
    ):
