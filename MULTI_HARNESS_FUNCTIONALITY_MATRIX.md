...
| # | Claude functionality | Codex | Antigravity | Opencode | Test artifact |
|---|---|---|---|---|---|
| 3.21 | `CLAUDE.md` project instructions | [DONE] `AGENTS.md`, one file serves all three (confirmed real auto-discovery for codex/opencode via their own docs) | [DONE] same file, but required a **real code fix** (`global_instructions_md` field) since agy doesn't auto-discover `AGENTS.md` on its own, unlike the other two | [DONE] same file | Start a fresh agent, ask "what does the task management section of your instructions say" — answer should reflect `AGENTS.md`'s actual content |
