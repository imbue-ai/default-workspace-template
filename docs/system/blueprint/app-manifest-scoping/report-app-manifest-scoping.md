...
Status as of 2026-09-10. Branch `mark/app-manifest-scoping`, three commits (`19be2c213` plan, `1c90bee0d` implementation, `494601659` review fixes), pushed. One end-to-end eval run against `1c90bee0d`, and a six-arm measurement of the two review invocations on a toy fixture.

## What was delivered

| Piece | Where | State |
|---|---|---|
| `[[references]]` and `[scope] exclude` in `app.toml`, validated by the library, ignored by registration | `system/libs/app_manifest/` | done, 168 unit tests |
...
Tests: library 168 passed; root suite 2,336 passed with the 9 known macOS-only `agy_shim` failures; the same suites pass on Linux in a container except tests needing `jq` (absent from that image). Changelog gate ok. CI has not run yet.
...
Three conditions per invocation: the control has no `[[references]]` in the manifest and the pre-branch `{creation_context}` text; the middle arm has the reference and the old text; the scoped arm has the reference and the scope brief.
...
- **Files read did not shrink; where they were read from did.** The scoped arms read 2 to 4 more files than the control, all inside the footprint (the ratchet file, `supervisord.conf`, the style guide, the scope file). Files outside the footprint fell from 3-4 to 1-2 on the old-text arms versus the scoped ones, and repo-wide greps from 3 to 1. The control arms are already small because the fixture is small; the reads the scope brief removes are the speculative ones (`update_self_test.py`, `agy_shim_test.py`, `pyproject.toml`, `uv.lock` in the old-text autofix arms).
...
So the answer to "what percentage of considered files did the manifest remove" on this fixture: none. The scope brief moved reads from outside the footprint (down 50-67%) to inside it (up 30-40%), cut repo-wide greps by two thirds, and cut the test run from the whole monorepo to the footprint. A fixture large enough to make the control's grep expensive, or with a consumer whose name does not contain the app's, is what would show a difference on files read and on criterion 1.
...
- Sidecar wiring uses a `program:<name>-*` prefix match; an app named `share` would absorb `share-gateway`.
