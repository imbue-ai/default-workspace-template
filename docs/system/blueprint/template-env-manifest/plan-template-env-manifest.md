...
2. **The "Open in Mind" call-to-action** -- no prose, just a large centered button plus a one-line copyable fallback beneath it. The button points at the HTTPS trampoline (`https://boweiliu.github.io/open-in-minds/?git_url=<repo url>`); a bare `minds://` link is never used, because GitHub renders it dead. The fallback line is `/use-template <repo url>`.
