...
- `models/Inventory.ts` replaced `AgentManager.ts` for the shell: the WebSocket client, `apps_updated`, `projects_updated`, `layout_op`, `load_layout`, `tab_rebound`, the `client_state` report, the address helpers (`addressFor`, `parseAddress`, `isAddressOfApp`), `findInstance`, `listInstances`, and `instancePageUrl` (the app's origin label on a workspace host, its registered loopback URL otherwise, with `{tab}` substituted).
