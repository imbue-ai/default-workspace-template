...
- `apps/minds_evals/imbue/minds_evals/minds_bridge.py`: the bridged calls (`/api/agents/create-chat`, `/api/agents`, `/api/agents/<id>/message`, `/api/agents/<id>/events`) target the chat app's loopback URL instead of port 8000. From the chat-agent split's phase 2, `/api/agents/create-chat` and `/api/agents/<id>/...` were aliases of `/api/chats/create` and `/api/chats/<chat-id>/...` (the id read as a chat id); phase 7 dropped the aliases, and the paired minds branch retargets the bridge to `/api/chats/create` (answering `chat_id`) and `/api/chats/<chat-id>/...`. `/api/agents` stays the plain listing of every mngr agent, beside `/api/chats`, which lists the chats as snapshots.
