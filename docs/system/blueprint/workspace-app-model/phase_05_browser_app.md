...
- `data_types.py` (`BrowserLifecycle`, `BrowserController`, `BrowserSnapshot`), `primitives.py` (`BrowserName`, the browser's name validated as its instance key; `derive_browser_title`; `instance_url_for_browser`), `errors.py` (the fleet's refusals: `FleetCreateRefusedError`, `UnknownBrowserError`, `BrowserNotDrivableError`, `BrowserHeldByAgentError`, `NavigationFailedError`).
