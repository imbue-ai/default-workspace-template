...
- `system/apps/chat/frontend/`: `package.json`, `vite.config.ts`, `index.html`, `src/` from the shell frontend's `src/chat/` plus the shared helpers it uses copied in (`base-path.ts`, `hooks.ts`, `origin.ts`, `slots.ts`, `style.css` tokens); builds into `imbue/chat/static/`.
...
- The provisional-instance flow, subagent instances, presence, the first-chat claim with the `first` template, and `/welcome` are unchanged from phase 6 in behaviour.
