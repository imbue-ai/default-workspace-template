...
The update notice is `{"merge_sha", "applied_at", "driven_by", "apps", "programs", "needs_services_restart", "progress", "outcome"}`: the record an apply run with `--keep-rollback-point` leaves at `data/.state/update-apply/last-good.json` (section 17), less its rollback target and copies. `apps` are the critical apps whose program or bundle the apply changed (their tabs carry the band; `system_interface` among them means the shell's own banner; an empty list, an apply that changed how the workspace starts and no app, is carried by the shell's banner alone, since no tab would), `programs` what a rollback restarts, `progress` what a running rollback is doing, and `outcome` how it ended; a notice with an `outcome` is settled and only closes.
