...
A preview shell (`system-interface --preview`, booted read-only over a seeded copy of the live state directory and a copied registry) answers every relay route above that acts on an instance it already has -- rename, location, stop, start, delete -- plus `POST /api/apps/<name>/stop` and `POST /api/apps/<name>/start`, with `403 {"detail": "This is a preview of a proposed change; it cannot change the live workspace."}`, and its page hides those verbs; its page carries the meta tag `system-interface-preview` (content `true`) and never the staleness tag. Project and layout edits land in its copy. Creating is the exception: `POST /api/apps/<name>/instances` and a create through the op route reach the app the copied registry names, so a preview whose sibling was booted with `--with` creates into that sibling's preview and one whose sibling was not creates a real instance of the live app. The New Tab page, the rail, and the All apps popover therefore work in a preview -- a preview of the shell with all three inert could not be clicked around at all -- and what a create makes is a new instance the user can delete rather than a change to one they already have.
