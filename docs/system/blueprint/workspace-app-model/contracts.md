...
| `wiring` | table | no | `{}` | `{programs = ["<program>", ...]}`: further `[program:*]` blocks under `system/supervisord.conf.d/` the app owns beyond its own and its `<name>-<role>` sidecars (the browser declares `xvfb`). Unique, never the app's own `program`; each must be declared when the footprint is computed. |
