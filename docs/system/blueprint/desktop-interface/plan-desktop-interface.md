...
The `connectToShell` signature gains `onNavigate` and a `capabilities` argument, and `location(path, title)` takes the title.
...
1. **Contract v2, additive.** `shell:location` gains `title`, `shell:capabilities` and `shell:navigate` are added, `shell:open` accepts a path beside the address it takes today, `connectToShell` gains the new handlers. The current shell ignores what it does not know, and apps keep the address form of `shell:open` until phase 5. Verify: the current shell still frames every app.
...
5. **The shell frontend.** The layered modules of section 6, the theme file, compact and touch modes, the launcher overlay hosting the New Tab sections, the two tray widgets; dockview and the rail deleted; the shell reads only the new routes; the chat page switches to the path form of `shell:open`. Verify by hand, on a laptop and on a phone: every behaviour of section 4 against the built-ins; a chat from a shortcut, from the launcher with a seeded message, from an agent, and from the minds "Ask an agent" path; a sub-agent view in its own window; two clients following one chat root's selection; two terminals and a reload reattaching; a files window reopening at its folder; a browser window following an agent's navigate.
6. **Deletion.** The shell's instance relay, tab route, projects, layouts, seeds, and migration; every app's instances API, nudge, `subagent` action, and sidecar; the `app_instances` library; `auto_open.py` retargeted to the op route's new `open`; ratchets tightened. Verify: `test_e2e.py` green with stub apps, and a fresh workspace boots to its default desktop.
