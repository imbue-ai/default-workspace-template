...
16. **The new agent receives as much context as could be relevant**, as its first message: the summary in full with its path (a summary over 64 KB is pointed at instead, since the send paths cap what one message can carry), or the note that none exists, the archived agent's identity and transcript locations, the closed steps, and the user's message. The create carried it as `--message` until phase 8, which made the create silent and sends it afterwards instead, so the model the user picked lands before the first turn (5.8).
...
22. **An agent whose first message carries no summary pointer, and that is not the first agent of its chat, gathers its own context** from its predecessors' transcripts.
    One line of `AGENTS.md` tells every agent so; the rest of what a successor does is in its first message.
...
24. **After a switch the chat runs on the model the user picked, else on what the switch implies**: a handoff's successor on the new harness's defaults, a rebind's agent on its own model, effort, and fast mode.
    A handoff reset them unconditionally until phase 8 gave the switch dialog a model picker, and a rebind refused a pick until phase 9 (5.1, 5.8, 6): the rebind's restart has the same seam between its start and its held sends that the handoff applies its pick in, so nothing but the refusal kept the user from changing account and model in one switch.
...
```json
{
  ...
  "sessions_dir": "/home/user/.minds/accounts/<previous, then the target once the files moved>",
  "restarted_account_id": null,
  "model_pick": {"model_id": "haiku", "effort": "high", "fast": false},
  "is_model_pick_applied": false,
  "trigger_message_id": "...",
...
The two entries share their common fields (`ChatTransitionRecord` in code): the phase, the target, the confirming message, the held sends, the returned queue, the error and its `failed_step`, and, since phase 9, the `model_pick` (`null` for none), which the handoff entry alone carried in phase 8.
...
The minds app's assist and update-self chats are created *through* the chat app since the branch that followed phase 4 (`system/scripts/message_chat.py --create`, posting to `/api/chats/create` with the window-surfacing labels and a wait for the create to finish), so they are chat-app chats like any other, `MINDS_CHAT_ID` included; only on a template whose script predates that mode does the minds app fall back to the bare create above.
...
```json
{
  ...
  "handoff": {"kind": "handoff", "phase": "summarizing", "started_at": "...", "target_lane": "openai",
              "target_account_id": "...", "target_harness": "codex", "target_label": "Codex",
              "held_sends": [{"message_id": "...", "text": "the confirming message"}],
...
- `handoff` is `null` except while converging (section 5.4, section 6): one shape for both kinds of switch, since the page renders them the same way.
  While set it carries `kind` (`handoff` or `rebind`), the phase (`restarting` is the rebind's), `started_at` (when the switch was confirmed, which since phase 8 is how the page tells this switch's summary request from an earlier, called-off switch's), the target lane, account, and harness, `target_label` (what the phase text and the 409s name the destination by: the harness for a handoff, the account for a rebind), the messages held for after the switch (the confirming message first, so a reloaded page keeps showing them until they land in the transcript; the record keeps the confirming text as `trigger_text` once it leaves the held list), and, since phase 9, `model_pick` (the model the switch is taking the chat to, `null` for none, which the model bar reads while it is being carried out, 5.12), and the failed phase's `error` and, since phase 8, `failed_step` (which step the retry reruns: `start` or `model`; a rebind's since phase 9).
...
- Confirming starts the handoff with the typed message as the first message for the new agent.
  The message shows as a placeholder whose text follows the backend's phase ("Wrapping up with Claude Code...", "Starting Codex...") rather than a bare "Sending...", because the transitions are long enough that the user should see what is happening.
...
- The typed message is painted as the page's own not-yet-real bubble carrying its `message_id`, which stands down the moment the snapshot lists the send under `handoff.held_sends`; from then on the held messages render from the snapshot with the phase as their caption ("Wrapping up with Claude Code...", "Claude Code is writing a summary...", "Starting Codex...", "Could not start Codex").
...
- Pressing an account on another harness opens the switch dialog at once rather than arming a "next" lane behind the menu: "Switch to Codex?" with a model picker for the successor (fed by `GET /api/accounts/<account_id>/model-options`).
...
As landed in phase 9, which lets a rebind take a model pick (section 6), revising the phase 8 statements above that a rebind's strip offers no Change and its card no Model row:

- Pressing an account on the chat's own harness and lane still arms the rebind at once, with no dialog. Opening the dialog on every rebind press was the alternative, and was not taken: a rebind usually keeps the model (it is the same harness and lane), so the common case stays one press, while the pick is one click away wherever the armed switch is shown.
- The strip above the composer offers Change for a rebind too, and the model bar's armed card a Model row; both open the dialog's rebind variant: "Switch to Anthropic 2 (Claude Code)?", "Claude Code restarts on Anthropic 2 (Claude Code) and keeps this conversation, starting with your next message.", and a picker whose first row, the default, is "Keep the current model", over the target account's models (`GET /api/accounts/<account_id>/model-options`). "Switch this chat" arms the pick with the rebind; "Start a new chat" opens a chat on that account, on the model picked, or on the model this chat runs on when none was.
- The dialog opened from Change, for either kind of switch, starts from the pick already armed for that account, so confirming it again does not drop the pick. Picking the armed account again from the provider chooser ("switch to another provider" under an auth-failed turn, or "+ Add a provider") keeps the pick the same way: a rebind stays armed with it, and a handoff's dialog starts from it.
...
- The dialog offers no picker for a harness whose model bar is read-only (antigravity, whose model is changed from the agent's terminal), for either kind of switch, and a new chat started from a rebind's dialog there does not carry the chat's model over: a pick made there could never be applied, and a rebind's retry would rerun it forever. The server holds the same line for any other caller: `GET /api/accounts/<account_id>/model-options` offers such a harness nothing, and a pick for one is rejected at the model step at once, without the retry a refused switch gets.

### 5.2 Trigger and preconditions

One switch route (`POST /api/chats/<chat_id>/handoff`, body `account_id`, `message`, `message_id`, the client fields a send carries, and, since phase 8, `model`, the pick the chat runs on after the switch, which a rebind takes too since phase 9) takes the chat id, the target account, and the message, and dispatches on the target account's harness and lane: the active agent's own harness and lane, on a harness listed in `harnesses/binding.py`'s `REBIND_VERIFIED_HARNESSES`, means a rebind (section 6); anything else means a handoff.
...
Step 3 fails into **failed** (5.10) when the create fails, and, since phase 8, when any other step of it is refused (a stop, the archival rename, the record write): past the point of no return no verb but destroy answers the chat and only the failed phase has a retry, so a refusal left in `switching` would wedge the chat until the next chat-app restart. A refusal in `draining` or `summarizing` keeps its phase, where cancel is still possible. The one exception is a successor the observe stream has not listed yet at delivery time, which is no refusal: the record keeps `switching` for the next resume.
...
A rebind keeps the agent's own model settings unless the user picks another in the dialog, which since phase 9 is applied once the agent is back and before its first message (section 6).
Since phase 9 the bar reads the picked model for the whole switch, rather than giving it up when the chat's identity flips and falling back to the live choice.
Neither flip means the pick has landed: a rebind relabels the agent with the target account partway through its restart (6), before the agent is started, and a handoff's successor becomes the chat's agent before the harness has written the model applied to it (5.4), so between them the pushed choice is the account the chat is leaving and then the model the new account last ran.
The pending lane therefore settles only once the switch is over, and hands the pick to the model bar's own optimistic overlay as it does, which holds it until the live choice matches it exactly as a pick made in the bar is held.
The pick also rides the chat snapshot (`handoff.model_pick`, 4.6), for a page with no pending lane to name it: one reloaded mid-switch.
...
- Sequence (`chat_rebinds.py`), on the record's `rebind` entry: `draining` returns the queue to the composer through the stop button's interrupt (the bounded wait for an in-flight send included), on the request's thread as a handoff's does; `restarting` then runs on a thread of the chat app's: `mngr stop` the agent, rewrite the binding in its own state dir, drop its resident watcher (only once the new binding is on disk, so a read that rebuilt it meanwhile is dropped too), `mngr label <agent> --label account=<new id>`, `mngr start <agent> --no-resume`, then, since phase 9, apply the model the user picked for it if they picked one, then deliver the held sends (the confirming message first) through the normal send path and clear the entry.
  The binding rewrite is the agent's harness binding's `rebind_agent`: the `CLAUDE_CONFIG_DIR=` line of the state dir's `env` file for claude (mngr sources that file into the tmux session on every start and never rewrites it), the credential link in the state dir for codex, pi, and antigravity (the same link their `create_args` repoints at create); each write lands whole, by a temp file or link renamed into place.
  Since phase 9 every per-harness difference in binding lives in that one class per harness (`harnesses/<harness>/account_binding.py`, registered on `HarnessSpec` as `binding_class` beside the watcher, tracker, and resolver classes): the scoping variable, the files that hold a signed-in credential (which a re-auth clears), the seeded account files, the create's arguments and the workspace's create defaults, the rebind's edit, and the session-file move below. Claude's is the env-file binding; codex, pi, and antigravity share a credential-link base whose session move is a no-op. The runner calls `move_sessions` and `rebind_agent` on whichever binding the agent's harness registers, with no harness check of its own, and `binding.py` keeps only the account resolution and `REBIND_VERIFIED_HARNESSES`.
...
- Claude keeps a chat's session files under the config dir it runs with, at `<account>/projects/<encoded work dir>/<session id>.jsonl` with the subagents under `<session id>/` beside it, and both mngr's resume chain and the chat app's watcher look them up under the current config dir.
  So before the env line changes, the rebind moves the agent's sessions (the ids in the state dir's `claude_session_id_history`, plus the agent's own uuid) from the previous account's tree to the new account's, keeping their place under `projects/` (`harnesses/claude/session_files.py`), and tracks the dir the files are under on the `rebind` entry (`sessions_dir`, harness-neutral since phase 9, which reads the `claude_sessions_config_dir` of an older record: the previous config dir, recorded before the rewrite, then the target once they moved; the claude binding decides both values and the record only keeps them), so a resume after the rewrite, or a retry on a third account after a failed start, still knows where the files are and moves them on.
...
- The confirm dialog's rebind variant was one line ("Claude restarts on Anthropic 2 (Claude Code) and keeps this conversation."); since phase 8 a rebind asks nothing when it is armed, since the agent keeps both its conversation and its model (5.1), and since phase 9 the dialog has a rebind variant again, opened from the armed switch rather than on the press, for picking the model (5.1). The phase text reads "Restarting Claude Code on Anthropic 2 (Claude Code)...", and the 409s name the account.
- The model pick (since phase 9) goes through the model bar's own path (`AgentManager.apply_model_pick`, validated against the agent's option set) once the start has landed and before the held sends, so it governs the agent's first turn back; with none, the agent keeps its model. `mngr start` returns without awaiting the harness, unlike the create a handoff's pick follows, so the first tries can meet an agent still coming up (codex's daemon answers the switch a few seconds after the start): a switch the harness refuses is tried again every second for up to thirty seconds, while a pick outside the agent's option set fails at once. A codex agent whose daemon has not answered yet is checked against the set it was last offered, on the account it left, so a pick outside that set is tried again too rather than rejected. A pick that cannot land fails the rebind at `failed_step: model`, with the agent up on the new account.
- There is no cancel: draining runs on the request's thread and the stop follows at once, so arming the switch is the last chance (before phase 8, the confirm was); the cancel route answers 409 with a detail that says so, and the page never shows "Cancel switch" for a rebind.
- A start that fails, or since phase 8 any step of the restart mngr refuses (the relabel included), leaves the chat in the `failed` phase at `failed_step: start` with mngr's output, the binding and the label already at the new account; the page's retry offers the accounts of the same harness and lane (the agent stays the chat's) and reruns the restart, every step finding its work done, then the pick; a retry after a pick that failed on the same account reruns only the pick and the deliveries, since the restart already landed there, and one on another account restarts first; the pick survives every retry, since a rebind's retry stays on its harness and lane; "Start a new chat instead" is the way out to anywhere else, and destroy remains available.
...
- Every step is idempotent and re-checked on resume (`_resume_handoffs` spawns the rebind runner beside the handoff runner): a stopped agent is not stopped again, a moved session file is not moved again, the env line and the link are rewritten to the same value, `mngr label` merges, and `mngr start` is a no-op for a running agent.
  The entry records the account the start landed on (`restarted_account_id`, written in the same update that moves the active entry), so a resume that finds it naming the target has only the pick and the delivery left; the active entry cannot serve as that marker, since it names the previous account until the start lands and a retry may name the previous account as its target.
  Since phase 9 it also records that the pick landed (`is_model_pick_applied`), so a resume or a retry does not type the pick into the session a second time; the agent keeps a pick that landed through any later restart, as it keeps every model setting.
...
- `AGENTS.md` gains one bullet every agent reads ("Continuing a chat that moved to you"): when `MINDS_CHAT_ID` is set and differs from `MNGR_AGENT_ID`, the agent is a successor in an existing chat and its first message says what to do; failing that, it lists its predecessors (`mngr list --include 'labels.chat_id == "$MINDS_CHAT_ID"'`) and reads their transcripts (`mngr transcript`); it never touches a predecessor and tells the user nothing of this unless asked.
  This is the backstop for a successor whose first message never arrived, and it costs one line when the message did. Everything else a successor does (read the summary, check `tk steps`) is in the `continue-chat` prompt itself, since phase 8, which cut the section down from a restatement of that prompt.
...
- The switch chip is the chat-level `agent_switch` event, synthesized between two segments with an id derived from the chat id and the retiring agent's sequence number; it carries `from_agent_id`, `to_agent_id`, `from_harness`, `to_harness`, and `seq`, plus `message_id` and `message` (the message the user switched with, kept on the successor's record entry as `opening_message` when the handoff folded it into the prompt; null for a fresh start, whose message is delivered as a turn of its own) and `is_fresh_start` (kept on the successor's record entry too; the page shows no handoff node for a fresh start), counts toward the chat's total, and opens the new agent's first section, on that message as the user's bubble when there is one. It rendered as a centered chip ("Switched from Claude to Codex") until phase 8, which folded it into the handoff node and drew a rule under the node in its place.
- The subagent routes resolve any member of the chat, archived ones through their loaders; the page builds a subagent view's key from the chat's active agent and the subagent's session (the instances API that once answered it is gone).
