...
- `handoff` is `null` except while converging (section 5.4, section 6): one shape for both kinds of switch, since the page renders them the same way.
  While set it carries `kind` (`handoff` or `rebind`), the phase (`restarting` is the rebind's), `started_at` (when the switch was confirmed, which since phase 8 is how the page tells this switch's summary request from an earlier, called-off switch's), the target lane, account, and harness, `target_label` (what the phase text and the 409s name the destination by: the harness for a handoff, the account for a rebind), the messages held for after the switch (the confirming message first, so a reloaded page keeps showing them until they land in the transcript; the record keeps the confirming text as `trigger_text` once it leaves the held list), and the failed phase's `error` and, since phase 8, `failed_step` (which step the retry reruns: `start` or `model`; a rebind's since phase 9).
