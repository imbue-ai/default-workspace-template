...
- The confirm dialog's rebind variant was one line ("Claude restarts on Anthropic 2 (Claude Code) and keeps this conversation."); since phase 8 a rebind asks nothing at all, since the agent keeps both its conversation and its model (5.1). The phase text reads "Restarting Claude on Anthropic 2 (Claude Code)...", and the 409s name the account.
