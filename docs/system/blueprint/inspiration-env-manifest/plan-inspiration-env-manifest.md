...
- **Append-only enforcement.** Three logs are documented append-only -- `## Inspirations` and `## Adopted inspirations` in `docs/VERSION_HISTORY.md`, and `Publication history` / `Adaptation history` in the manifest -- and nothing enforces it. The only script that touches `VERSION_HISTORY.md` is `build_inspiration.sh`, and only to `rm -f` it out of the snapshot; the single weak check is `publish-inspiration` §8 step 4's per-slug idempotence test. Options, cheapest first: (1) a pre-commit/CI check that diffs to those paths contain no `-` lines; (2) an append-only writer CLI so no agent hand-edits the files; (3) move the logs to JSONL and render markdown from them, matching `env_converge`'s `events.jsonl` and the style guide's append-only event conventions. Awaiting the user's choice; nothing is built until then.
...
- **Phase 5 -- append-only enforcement.** Conditional on the user's choice above.
