...
## Eval results (post-implementation notes)

The sections above are the plan as written. The comparison ran on 2026-09-09 and 2026-09-10 from the mngr-internal worktree (`apps/minds_evals/nested-worker-dispatch-runs.md` there lists the job directories and per-trial numbers).

- **Every nested dispatch worked.** In all four parallel trials the harden worker launched two siblings, awaited both with 60m timeouts, answered no questions (none were raised), merged both with `--no-ff`, stopped them, and reported `done`; the chat lead's `await` never misread the waiting harden worker as idle. Workers parsed their task files at the stamped path and reported with the `report` subcommand; every report was archived once. Every launched worker scored 1.0 on harness soundness, and every trial passed every gate with no timeouts.
- **Parallel is slower on the todo app, not faster.** Direct hardening took 20 to 33 minutes (mean 27); parallel took 52 to 93 minutes (mean 68). Provisioning is not the cost (15 to 30 seconds per worker inside a workspace). The cost is the lead's serial phases around the siblings: deciding the split (7 to 10 minutes), then merging and re-running the full verification (17 to 21 minutes), which together roughly equal a whole direct pass on a creation this small. The siblings themselves ran 11 to 31 minutes in parallel. This answers the first open question: the "weigh the cost" paragraph in the harden contract is the right default, and a split pays only when the sibling work is long relative to a full verification run.
- **No quality loss, and a suggestive gain.** The outcome judge scored the delivered app 1.0 on all three parallel attempts against 0.33, 0.67, and 0.44 on the direct ones. Three attempts of a sampled judge are not a proof, but the parallel arm never scored below the direct arm on any dimension.
- **A gate-policy drift surfaced.** The harden contract's "Review gates" section defines tests and ratchets only; the code-guardian gates were removed from it deliberately, and `verification.md`, which still describes them, is not in the worker's routing table. The parallel section originally said "both review gates" and one parallel lead spent 40 minutes on them; it now says the lead runs exactly the verification a direct pass runs. Whether the code-guardian gates should return to the contract is a separate decision.
- **The second open question stays open.** No sibling raised a `question` gate in any trial, so how a re-raised question behaves under the eval's polling prompt is still unobserved.
