...
- The chat: `relay.test.ts` forwards `shell:draft-text`; `elementReferences.test.ts` spills only oversize reference blocks and survives a failed post; `element_references_test.py` writes the file whole and private, refuses anything but one object under the key, and reports an unwritable directory; `server_test.py` covers the route's answer, its `400`, and its `500`.
