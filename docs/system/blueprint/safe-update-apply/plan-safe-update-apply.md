...
- New environment snapshots: copy-aside/restore for root `.venv`, both uv tool environments (resolved as the refresh resolves its install target — the console script's shebang and receipt, else the mngr tool's directory, else the build's pinned tool home — so what is copied aside is what the reinstall rebuilds), and frontend `node_modules`; restores are plain copies back to the same absolute paths.
...
Open questions, review residue, and the incident record for this work are tracked in one place, in the mngr-internal repo: `blueprint/inner-workspace-updates/open-threads.md` (the paired Mind-app plan lives beside it).
