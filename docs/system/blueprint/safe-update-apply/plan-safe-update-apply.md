...
- New `run-status` subcommand (`start` / `verdict <VERDICT>`): the whole machine-readable status contract between an update-self pass and the Minds app, one `RunStatus` JSON record at `data/.state/update-apply/run.json`. The lead records the run's start (chat agent name from `$MNGR_AGENT_NAME`, unattended flag) before anything else in the pass, and exactly one terminal verdict (`UPDATED`, `UPDATED_WITH_REBUILD_ITEMS`, `ALREADY_CURRENT`, `NEEDS_RECREATION`, `STUCK`, `REFUSED`, plus a plain-language detail line and optional resulting / in-place-compatible refs) from whichever SKILL.md section ends the pass. The app polls the file over `mngr exec` alongside the marker and the run's chat agent — there is no event stream — so a run recorded here is visible to the app whoever launched it, and a new run's `start` supersedes the previous run's record.
