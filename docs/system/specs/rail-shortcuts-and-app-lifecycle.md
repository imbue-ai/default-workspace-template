...
Related reading: the system_interface README's "Projects" section (the verb set and rail as they are today), `frontend/src/views/objectMenu.ts` (the object verb set this spec extends), `.agents/skills/manage-projects/SKILL.md` (the agent-facing project/view layer), and `blueprint/random-service-labels/` (why registry labels are stable).
