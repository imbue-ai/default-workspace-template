...
- `worker` - For sub-agents created via the launch-task skill and the flows built on it (the crystallize / update / heal creation lifecycle, including update-system-interface). Includes code review. Nothing is installed into the worker's own `.agents/skills/`: a worker running a harden pass reads the generic worker contract from `.agents/shared/worker/SKILL.md`, which the checkout in its worktree already carries.
