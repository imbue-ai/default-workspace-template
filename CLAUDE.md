...
2. `tk create --step "..."` for every step you currently expect, in order. You may batch them into one tool call, but each step must be its **own separate `tk create` command** (on its own line or joined with `;`) — do NOT pass multiple `--step`s to one `tk create` (it makes only one step, silently keeping just the last title), and do NOT redirect the output (the progress view reads each new step from the visible `Created <id>: <title>` line). Note the ids. Do NOT `tk start` any yet.
