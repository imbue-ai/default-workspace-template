...
- If you need to *actively develop* against an external repo (e.g. `mngr`), check out a standalone clone of it under `.external_worktrees/<repo-name>/`. This directory is gitignored so the external clones don't pollute the monorepo. The branch in the external clone should mirror the branch you're on in this monorepo. For mngr specifically, follow `.agents/skills/submit-upstream-changes/references/mngr-changes.md`, which submits them as their own mngr PR; mngr is installed here from the commit `pyproject.toml` pins (`[tool.uv.sources]`), or from the local tree at `system/vendor/mngr` in a workspace built against an mngr checkout -- neither is a git checkout you can commit from.
