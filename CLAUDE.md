...
- If you need to *actively develop* against an external repo (e.g. `mngr`), check out a standalone clone of it under `.external_worktrees/<repo-name>/`. This directory is gitignored so the external clones don't pollute the monorepo. The branch in the external clone should mirror the branch you're on in this monorepo. For mngr specifically, follow `.agents/skills/submit-upstream-changes/references/mngr-changes.md`, which submits them as their own mngr PR; mngr is installed here from the commit `pyproject.toml` pins (`[tool.uv.sources]`), not from a tree you can edit in place.
...
- To run tests for a single project: "cd system/apps/system_interface && uv run pytest". Each project has its own pytest and coverage configuration in its pyproject.toml. mngr's own tests live in its repo, not here.
...
If you get a "ModuleNotFoundError" error for a 3rd-party dependency when running a command that is defined in this repo (like `mngr`), which refresh fixes it depends on *which* install you ran, because a standard workspace has two: `uv run mngr ...` resolves from the root venv, and a bare `mngr` on PATH is the uv-managed tool that `system/scripts/build_workspace.sh` installs at build time. For the venv one, run `uv sync --all-packages`. For the tool one, rebuild it exactly as `system/scripts/build_workspace.sh` does -- mngr and every plugin from the pinned commit, in one command so uv resolves them together: `uv tool uninstall imbue-mngr; uv tool install "$(python3 system/scripts/list_mngr_plugins.py --base)" $(python3 system/scripts/list_mngr_plugins.py --tool mngr | sed 's/^/--with /')`. Reinstalling from the base package alone would drop the plugins and leave a tool that cannot parse its own plugin config; `uv tool list` shows what is installed. Then try running the command again.
