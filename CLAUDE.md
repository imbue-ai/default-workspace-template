...
Before proactively suggesting permissions, sharing, inspirations, driving the browser, or personalizing the agent (CLAUDE.md, skills, look and feel), check `data/.state/user_knowledge.toml` so you don't re-suggest something the user already knows about or has repeatedly ignored -- see that file's own doc comment (or `data/.state/README.md`) for how each capability's "already knows about it" fact is determined (some are self-reported by the skill that performs the action; permissions and personalization are checked live instead, against latchkey and the repo's own history respectively, since a locally cached flag could go stale). When you do make one of these suggestions, record it: bump `times_suggested` and set `last_suggested_at` (`YYYY-MM-DD`) under that capability's table in the same file, creating it if absent. Back off after a couple of ignored suggestions rather than repeating it every session.
