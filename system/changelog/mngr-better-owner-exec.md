- Replaced the in-container Python `owner-exec` service with the pinned Go
  binary from `imbue-ai/owner-exec` (installed at image build via
  `system/scripts/install_owner_exec.sh`, run by supervisord through
  ...
  The endpoint surface (`/run`, `/read-file`, `/write-file`, `/grants` with CAS,
  `/_alive`) is unchanged.
...
- Added a one-shot `vm-exec-register` supervisord program
  ...
  the same way it drives the container. A no-op on local docker/lima workspaces.
