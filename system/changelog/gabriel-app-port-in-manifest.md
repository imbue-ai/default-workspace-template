...
This makes the manifest the one place an app's port is written, which is what lets
the build-app port pre-flight and migrate-workspace's port scan account for apps
whose supervisord command names no port at all.
