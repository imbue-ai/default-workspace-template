...
1. An oom self-tag. The agent's subprocesses (builds, test runs, browsers) are
   ...
   is, belong to ``oom_priority.bands.oom_tag_shell_prefix``, which the update
   apply (``update_banding.py``) uses too -- to band its own hungry forward
   steps down out of the protection it keeps for itself.
