...
_link_playwright_chromium() {
    ...
    #
    # Fast satisfied-check: some Playwright cache entry already resolves to Fortress.
    # The glob tolerates Playwright's version-suffixed directory names; resolving the
    # exact path through Playwright itself (below) costs ~a second, so only do that
    # when the link is absent. A separate step from _install_fortress so it also
    # converges on workspaces whose Fortress predates this link.
    local existing
    for existing in "${HOME:-/root}"/.cache/ms-playwright/chromium-*/chrome-linux*/chrome; do
        if [ -L "$existing" ] && [ "$(readlink -f "$existing")" = "$_FORTRESS_INSTALL_DIR/tilion-fortress/tilion" ]; then
            _log "fortress: Playwright default chromium already links to Fortress, satisfied"
            return 0
        fi
    done
    local pw_chrome
