...
#
# Two callers construct a minimal environment and drop the agent env: cron
# (roughly just HOME/PATH/SHELL/LOGNAME) and sshd, which builds a fresh
# environment per session -- so neither `17 3 * * * my_job.sh` nor
# `ssh <host> '<command>'` sees MNGR_HOST_DIR, MNGR_AGENT_ID, LATCHKEY_*, or the
# PATH that puts uv at /root/.local/bin. All of it is reconstructable from files
# mngr maintains on the host dir, so this wrapper rebuilds it the same way mngr
...
# given command.
#
# Restoring the env per-command is deliberate, rather than publishing it once to
# somewhere every session reads (/etc/environment via pam_env, say). The agent
# env carries both credentials and MNGR_AGENT_ID -- and mngr's teardown scans
# /proc for that id to find an agent's orphaned processes, so an ssh session
# holding it is indistinguishable from one and dies with the agent on `mngr
# stop`. Only the command that asked for the env gets it.
#
...
#
#   17 3 * * *   root   /home/user/workspace/system/scripts/with_agent_env.sh bash my_job.sh >> /var/log/supervisor/my-job.log 2>&1
#
# So should any command sent over ssh that needs the workspace's own tooling:
#
#   ssh <host> '/home/user/workspace/system/scripts/with_agent_env.sh uv run host-backup-now'
set -euo pipefail
