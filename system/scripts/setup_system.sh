...
# Download with hardened retries. Release CDNs (GitHub's especially) reset
# connections for minute-plus windows from cloud egress (observed as
# `curl: (56) Connection died` failing the whole provision under `set -e`).
# Plain --retry does NOT retry connection resets -- curl only deems timeouts
# and a few 4xx/5xx codes transient -- so --retry-all-errors is load-bearing;
# --retry-max-time bounds the whole retry window so a hard outage still fails
# the build in bounded time. Writes to stdout unless -o is passed.
curl_fetch() {
    curl -fsSL --retry 8 --retry-all-errors --retry-max-time 300 --connect-timeout 20 "$@"
}
...
curl_fetch "https://github.com/restic/restic/releases/download/v${RESTIC_VERSION}/restic_${RESTIC_VERSION}_linux_${restic_goarch}.bz2" -o /tmp/restic.bz2
...
curl_fetch "https://github.com/cli/cli/releases/download/v${GH_VERSION}/gh_${GH_VERSION}_linux_${gh_goarch}.tar.gz" -o /tmp/gh.tar.gz
...
curl_fetch "https://github.com/caddyserver/caddy/releases/download/v${CADDY_VERSION}/caddy_${CADDY_VERSION}_linux_${caddy_goarch}.tar.gz" -o /tmp/caddy.tar.gz
...
curl_fetch "https://github.com/fatedier/frp/releases/download/v${FRP_VERSION}/frp_${FRP_VERSION}_linux_${frp_goarch}.tar.gz" -o /tmp/frp.tar.gz
...
curl_fetch "https://astral.sh/uv/${UV_VERSION}/install.sh" | sh
...
curl_fetch https://claude.ai/install.sh > /tmp/install_claude.sh
