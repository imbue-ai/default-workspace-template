...
@pytest.mark.parametrize(
    "stored_url",
    [
        # Local shape: the service name is the host's first dot-label.
        "http://api.agent-0af.localhost:8421/health",
        # Shared shape: the service name is the first ``--`` token.
        ...
        "panels": [
            {
                "ref": "service:web",
                "panel_type": "iframe",
                "url": "http://web.agent-0af.localhost:8421/",
                "title": "web",
            },
        ],
        "tree": {
...
def test_service_coordinates_from_url_handles_both_host_shapes() -> None:
    """The predicate's URL parser understands both service origin spellings
    and rejects non-service URLs (external panels, the bare workspace
    origin) so a ``service:`` expectation can never match them."""
    assert layout._service_coordinates_from_url("http://api.agent-0af.localhost:8421/health") == ("api", "/health")
    assert layout._service_coordinates_from_url("http://web.agent-0af.localhost:8421/") == ("web", "/")
    assert layout._service_coordinates_from_url("https://api--myhost--user.example.com/health") == ("api", "/health")
    assert layout._service_coordinates_from_url(
        "http://browser.agent-0af.localhost:8421/?session=2"
    ) == ("browser", "/?session=2")
