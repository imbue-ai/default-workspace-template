...
- Persistent state (anything written and read across runs -- cursors,
  ...
  (defined below), never a hardcoded ``data/.apps/keyfoundry-mockdeck/`` at the
  call site. ``DATA_DIR`` defaults to ``data/.apps/keyfoundry-mockdeck/`` but
  honors the ``KEYFOUNDRY_MOCKDECK_DATA_DIR`` env var, so an editing agent can point a
...
- Listen port: bind ``PORT`` (defined below), which defaults to this
  app's assigned port but honors the ``KEYFOUNDRY_MOCKDECK_PORT`` env var, so
...
``http://keyfoundry-mockdeck.<workspace-host>/`` straight to this port), so it serves
...
# ``data/.apps/keyfoundry-mockdeck/`` but is overridable via the ``KEYFOUNDRY_MOCKDECK_DATA_DIR`` env var
...
# never hardcode ``data/.apps/keyfoundry-mockdeck/`` at a call site, or the override is
# bypassed. A writing call site should ``DATA_DIR.mkdir(parents=True,
# exist_ok=True)`` before writing.
DATA_DIR = Path(os.environ.get("KEYFOUNDRY_MOCKDECK_DATA_DIR", "data/.apps/keyfoundry-mockdeck"))

# Listen port. Defaults to this app's assigned port but is overridable via
# the ``KEYFOUNDRY_MOCKDECK_PORT`` env var so an editing agent can boot a throwaway
...
PORT = int(os.environ.get("KEYFOUNDRY_MOCKDECK_PORT", "8097"))

app = Flask("keyfoundry_mockdeck", static_folder=None)

_DECK_HTML = (Path(__file__).parent / "assets" / "deck.html").read_text()
...
# reopen this app's tab at the same place. Injected into the bundled deck
...
_DECK_HTML = _DECK_HTML.replace("</body>", _LOCATION_BEACON + "</body>")


@app.route("/")
def index() -> Response:
    return Response(_DECK_HTML, mimetype="text/html")
