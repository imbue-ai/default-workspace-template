...
beacon that posts the path being viewed and the folder's name one hop up
(`window.parent.postMessage({type: "shell:location", path, title})`, the message
of contracts section 10; the title is the last segment of the folder shown, or
`Files` at the served root) on each page load, so the workspace shell can reopen
a file-viewer instance at the folder it was looking at and title its window
after the folder. Hiding is purely
