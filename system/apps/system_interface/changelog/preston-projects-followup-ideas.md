...
**The rail folds up when the pointer leaves the window.** It follows the pointer everywhere inside the page already, but a cursor leaving the window does not reliably announce itself to the page, so the expanded rail could be left floating over the dock while the pointer was on another application entirely. An open menu or a row mid-rename still holds it open, exactly as they do for an ordinary pointer-leave.
