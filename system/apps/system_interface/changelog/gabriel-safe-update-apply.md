...
The server records the tree HEAD it started from and stamps an `X-Workspace-Update-Staleness` header (and meta tag) on app-shell responses when the live tree has moved under it -- `updated-not-activated`, or `update-interrupted` when the update apply's marker is present -- and a small informational banner renders the two messages, pointing the user at their agent.
