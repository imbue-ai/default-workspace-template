...
The cause was the sign-in check. On every page load the system interface runs `claude auth status`, and it kills that command if it overruns its budget. The `claude` CLI opens the terminal device directly, even with its output redirected, and it restores terminal modes while shutting down. Because supervisord leaves every service it starts in a background process group on the workspace's terminal, that restore made the kernel stop the whole group -- the system interface along with the command it was killing. The socket kept accepting connections and nothing ever answered them.
