 ...
 * - on shared hostnames (same rule, longer coordinate):
...
/** A workspace host-coordinate label: ``host-<hex>`` (``agent-`` is the legacy
 *  spelling of the same coordinate). Service labels can never match this --
 *  ``host-``/``agent-`` prefixes are reserved in forward_port.py. */
const WORKSPACE_COORDINATE_LABEL = /^(?:host|agent)-[a-f0-9]+$/i;

/** The workspace host coordinate within ``host``: the ``host-<hex>`` label and
 *  everything after it (``host-<hex>.localhost:8421`` locally,
 *  ``host-<hex>.<user>.<region>.<domain>`` on a share), with any leading
 *  service label(s) stripped. Returns ``host`` unchanged when it carries no
 *  coordinate label (a non-workspace host), so the derivation degrades safely. */
