...
describe("the small helpers", () => {
  it("read a client, spell a shortcut key, and compare frames", () => {
    ...
    expect(isSameFrame({ x: 0, y: 0, width: 1, height: 1 }, { x: 0, y: 0, width: 1, height: 1 })).toBe(true);
    expect(isSameFrame({ x: 0, y: 0, width: 1, height: 1 }, { x: 0, y: 0, width: 1, height: 0.5 })).toBe(false);
  });
});
