...
// retires it in the fleet (DELETE /service/browser/browsers/<name>) rather than
// killing an mngr agent or a tmux session.
...
async function executeBrowserDestroy(name: string, panelId: string): Promise<void> {
  // Retire the browser in the fleet via the browser app's DELETE route, then drop
  // the tab. Closing the tab alone only detaches the pane; this frees the browser
  // (and forgets its profile), symmetric to destroying an agent or a terminal.
  try {
    const response = await fetch(getServiceUrl("browser") + `browsers/${encodeURIComponent(name)}`, {
      method: "DELETE",
    });
    if (!response.ok) {
      const data = await response.json().catch(() => ({}));
