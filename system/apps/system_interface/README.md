...
gate on that same branch; then the change is merged and applied to the live UI
with auto-rollback. See `.agents/skills/update-system-interface/SKILL.md`.
...
Refreshing a live preview in place is **not** a sub-command here: it is the
shared `serve_isolated_instance.py`'s own `refresh --name si-preview-<slug>`,
which re-boots the preview's inner app on its existing port to pick up a backend
edit during the live loop, without disturbing the wrapper frame or the user's
tab (a frontend-only round needs no bounce -- just rebuild and `layout.py
refresh si-preview`).

A second system interface on this host cannot run its own `mngr observe` (the
live one holds the single-writer lock), so the preview -- and the update apply's
pre-flight boot -- launch with `SYSTEM_INTERFACE_AGENT_EVENTS_MODE=FOLLOW`,
reading the live observer's event stream instead, and gate on the strict
`/api/health`, which stays red unless that stream is really feeding the
instance (`/api/agents` runs its own discovery and answers 200 even on an
instance whose agent view is frozen). See "Agent lifecycle events" below.
