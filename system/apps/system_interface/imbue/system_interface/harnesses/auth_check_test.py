...
pattern), the fail-closed behavior, and the opencode regex guard are all checked without any
harness installed.
"""
...
def test_opencode_regex_treats_zero_credentials_as_signed_out() -> None:
    pattern = HARNESS_AUTH_CHECKS[HarnessType.OPENCODE].unauthenticated_output_pattern
    assert pattern is not None
    assert _is_signed_in(_pattern_check("0 credentials", pattern)) is False


def test_opencode_regex_treats_ten_credentials_as_signed_in() -> None:
    """The ``[^0-9]`` guard keeps ``10 credentials`` from matching the zero-credential case."""
    pattern = HARNESS_AUTH_CHECKS[HarnessType.OPENCODE].unauthenticated_output_pattern
    assert pattern is not None
    assert _is_signed_in(_pattern_check("10 credentials", pattern)) is True
