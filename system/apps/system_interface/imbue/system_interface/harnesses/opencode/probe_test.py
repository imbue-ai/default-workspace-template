...
def test_startup_model_prefers_a_pinned_config_model() -> None:
    assert (
        startup_model_from_config(
            {"model": "anthropic/claude-sonnet-4-5"}, {"default": {"opencode": "big-pickle"}}
        )
...
def test_startup_model_falls_back_to_provider_default_when_unpinned() -> None:
    ...
    assert startup_model_from_config({}, {"default": {"anthropic": "claude-sonnet-4-5"}}) == "anthropic/claude-sonnet-4-5"
