"""Placeholder transcript watcher for antigravity (agy) agents.

This first cut wires antigravity into the UI for the **model bar** only -- it does
NOT yet render agy's conversation. So the watcher is a no-op that satisfies the
:class:`AgentSessionWatcher` contract with an empty transcript: an antigravity tab
shows no messages until the real watcher (tailing agy's raw transcript at
``logs/antigravity_transcript/events.jsonl``) replaces this. Everything else -- the
model bar's catalog and live ``model_choice`` -- works regardless, since those flow
through the resolver, not the watcher. Mirrors ``pi_coding``'s placeholder.
"""
...
class AntigravityPlaceholderSessionWatcher(AgentSessionWatcher):
    """A watcher that emits nothing -- agy's transcript is not wired up yet."""

    _agent_id: str
    _on_events: Callable[[str, list[dict[str, Any]]], None]

    @classmethod
    def build(cls, agent_info: AgentInfo, on_events: OnEventsCallback) -> "AntigravityPlaceholderSessionWatcher":
