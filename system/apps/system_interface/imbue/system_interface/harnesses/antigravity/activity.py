...
agy's transcript carries no turn-boundary markers (like claude, unlike codex), so
...
derives IDLE; it becomes meaningful once the real agy transcript watcher lands.
Mirrors ``pi_coding``'s tracker.
"""
...
class AntigravityActivityTracker(HarnessActivityTracker):
    """antigravity: no turn markers, so activity is inferred from the mngr lifecycle plus
    the transcript tail (the same derivation claude uses)."""

    # mngr touches this on launch/resume; its mtime bounds transcript staleness. Inert while
    # the placeholder watcher emits no events (activity always derives IDLE); it matters once
    # the real transcript watcher lands.
