...
Each non-claude harness (codex, antigravity, pi, opencode) authenticates through its own
...
- antigravity: ``agy models`` exits non-zero when signed out.
- pi:          ``pi --list-models`` prints ``No models available`` (still exits 0), so the
               signal is in the output.
- opencode:    ``opencode auth list`` prints ``0 credentials`` (still exits 0).
...
HARNESS_AUTH_CHECKS: dict[HarnessType, HarnessAuthCheck] = {
    ...
    HarnessType.ANTIGRAVITY: HarnessAuthCheck(
        command=("agy", "models"),
    ...
    HarnessType.OPENCODE: HarnessAuthCheck(
        command=("opencode", "auth", "list"),
        display_name="OpenCode",
        unauthenticated_output_pattern=r"(^|[^0-9])0 credentials",
    ),
}
