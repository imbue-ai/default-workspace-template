...
def test_claude_tool_labels(tool_name: str, input_preview: str, expected: tuple[str, str]) -> None:
    assert claude_tool_labels(tool_name, input_preview) == expected
...
def test_bash_prefers_the_agents_description_over_the_raw_command() -> None:
    ...
    _, caption_label = claude_tool_labels("Bash", '{"command":"uv run pytest -q","description":"Run the tests"}')
    assert caption_label == "Running Run the tests"


def test_bash_falls_back_to_the_command_when_undescribed() -> None:
    _, caption_label = claude_tool_labels("Bash", '{"command":"ls -la"}')
...
def test_target_key_order_is_load_bearing() -> None:
    ...
    _, caption_label = claude_tool_labels("WebFetch", '{"url":"https://example.com","description":"read it"}')
...
def test_an_unparseable_preview_degrades_to_the_bare_verb(input_preview: str) -> None:
    ...
    assert claude_tool_labels("Read", input_preview) == ("Tool: Read", "Reading…")


def test_unknown_tool_with_a_target_still_says_something_useful() -> None:
    assert claude_tool_labels("SomeNewTool", '{"path":"x/y.txt"}') == ("Tool: SomeNewTool", "Running y.txt")


def test_unknown_tool_without_a_target_is_generic() -> None:
    assert claude_tool_labels("SomeNewTool", "{}") == ("Tool: SomeNewTool", "Running tool…")


def test_mcp_tool_keeps_its_raw_name_in_the_header() -> None:
    header_label, caption_label = claude_tool_labels("mcp__deepwiki__ask_question", "{}")
...
def test_mcp_server_name_may_itself_contain_the_separator() -> None:
    ...
    _, caption_label = claude_tool_labels("mcp__plugin_playwright_playwright__browser_click", "{}")
