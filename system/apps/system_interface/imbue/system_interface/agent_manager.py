...
def _refuse_to_set_oom_score_adj(pid: int, adj: int) -> bool:
    ...
    ``update-system-interface`` preview, the update apply's pre-flight) may contribute to:
    ...
    is not the workspace's, or for the headless update apply's pre-flight nothing at all,
...
class AgentManager:
    ...
    def build(
        ...
        an open port.
        ``events_mode`` selects whether this instance
