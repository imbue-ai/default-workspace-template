...
def _register_client(
    app: Flask, client_id: str, view_id: str
...
def test_an_app_nudge_is_accepted_from_loopback_only(
    ...
    try:
        ...
        assert (
            client.post(
                "/api/apps/terminal/changed", environ_base=_NOT_LOOPBACK
            ).status_code
            == 403
        )
    finally:
...
def test_a_tab_report_rebinds_the_tab_everywhere_and_files_it_in_the_project(
    ...
    shell.layouts.save_browser_layout(
        "alpha", "c1", layout_showing(_TERMINAL_1), None, TEST_NOW
    )
    shell.layouts.save_browser_layout(
        "everything", "c2", layout_showing(_TERMINAL_1), None, TEST_NOW
    )
    fetcher.list(
        TEST_TERMINAL_URL, instance_record("terminal-1"), instance_record("terminal-2")
    )
    ...
    assert (
        shell.layouts.read_layout("alpha", "c1", DeviceKind.DESKTOP).tabs["p0"].address
        == _TERMINAL_2
    )
    assert (
        shell.layouts.read_layout("everything", "c2", DeviceKind.DESKTOP)
        .tabs["p0"]
        .address
        == _TERMINAL_2
    )
    ...
    assert (
        rebound[0]["address"] == str(_TERMINAL_2)
        and rebound[0]["tab_id"] == "tab-0000000000000000"
    )
    assert [message["type"] for message in messages][-1] == "apps_updated"
    assert shell.inventory.find_instance(_TERMINAL_2) is not None


def test_a_tab_report_is_refused_when_it_names_no_tab_or_the_wrong_app(
    client: FlaskClient, app: Flask
) -> None:
    _shell(app).layouts.save_browser_layout(
        "everything", "c1", layout_showing(_FILES), None, TEST_NOW
    )
    assert (
        client.post(
    ...
    assert (
        client.post(
            "/api/tabs/nope/instance", json={"app": "terminal", "key": "k"}
        ).status_code
        == 400
    )
    assert (
        client.post(
            "/api/tabs/tab-0000000000000000/instance", json={"app": "terminal"}
        ).status_code
        == 400
    )
    assert (
...
def test_client_activity_is_appended_by_kind(client: FlaskClient, app: Flask) -> None:
    ...
    assert (
        client.post("/api/client-activity", json={**base, "kind": "nope"}).status_code
        == 400
    )
    assert (
        client.post(
...
def test_instance_verbs_are_relayed_and_the_list_refetched(
    ...
    created = client.post(
        "/api/apps/stub/instances", json={"action": "new", "params": {}}
    )
    assert (
        created.status_code == 201 and created.get_json()["instance"]["key"] == "stub-2"
    )
    assert inventory.find_instance(Address("app:stub?instance=stub-2")) is not None
    renamed = client.post(
        "/api/apps/stub/instances/stub-2/rename", json={"title": "Renamed"}
    )
    ...
    assert (
        client.post(
            "/api/apps/stub/instances/stub-2/location", json={"path": "/deeper"}
        ).status_code
        == 200
    )
    ...
    assert (
        client.post(
            "/api/apps/stub/instances/stub-9/rename", json={"title": "x"}
        ).status_code
        == 404
    )
    assert (
        client.post(
            "/api/apps/unknown/instances", json={"action": "new", "params": {}}
        ).status_code
        == 404
    )
    ...
    assert (
        client.post(
            "/api/apps/stub/instances/-not-a-key/rename", json={"title": "x"}
        ).status_code
        == 400
    )
...
def test_stop_and_start_drive_the_supervised_program(
    ...
    registry_path = write_two_app_registry(
        ...
        registry_row_toml(
            "chat", "http://localhost:8000", True, program="system_interface"
        ),
        registry_row_toml(
            "plain",
            "http://localhost:1",
    ...
    inventory = build_inventory(
        registry_path, broadcaster, prober=probe_all_app_liveness
    )
    client = shell_application(tmp_path, inventory, broadcaster).test_client()
...
def test_projects_are_created_seeded_and_listed(
    client: FlaskClient, app: Flask
) -> None:
    client_queue = _register_client(app, "c1", "everything")
    created = client.post(
        "/api/projects", json={"name": "Research", "color": "#12B5A5", "glyph": 4}
    )
    ...
    assert (
        client.post(
            "/api/projects", json={"name": "Bad", "color": "red", "glyph": 4}
        ).status_code
        == 400
    )
    assert client.post("/api/projects", json={"name": "Bad"}).status_code == 400
    assert [message["type"] for message in drain_messages(client_queue)] == [
        "projects_updated"
    ]


def test_project_settings_tabs_shortcuts_and_deletion(
    client: FlaskClient, app: Flask
) -> None:
    ...
    assert (
        client.post(
            "/api/projects/alpha/tabs", json={"address": "terminal:terminal-1"}
        ).status_code
        == 400
    )
    removed = client.post(
        "/api/projects/alpha/tabs/remove", json={"address": str(_TERMINAL_1)}
    )
    ...
    pruned = client.post(
        "/api/projects/alpha/shortcuts/remove", json={"app": "files", "action": "open"}
    )
    assert [shortcut["app"] for shortcut in pruned.get_json()["shortcuts"]] == [
        "terminal"
    ]

    deleted = client.post("/api/projects/alpha/delete")
    assert deleted.status_code == 200 and deleted.get_json() == {
        "fallback_view_id": "beta"
    }
...
def test_layouts_are_read_per_client_with_the_seed_as_fallback(
    client: FlaskClient, app: Flask
) -> None:
    assert client.get("/api/layouts/missing?client=c1").status_code == 404
    assert client.get("/api/layouts/everything").status_code == 400
    assert (
        client.get("/api/layouts/everything?client=c1&device=tablet").status_code == 400
    )
    ...
    assert (
        client.post("/api/layouts/everything", json={"client_id": "c1"}).status_code
        == 400
    )
    assert (
        client.post(
            "/api/layouts/everything", json={**body, "save_id": "nope"}
        ).status_code
        == 400
    )
    ...
    assert (
        own["tabs"]["p0"]["address"] == str(_TERMINAL_1)
        and own["updated_at"] == saved.get_json()["updated_at"]
    )
    seeded = client.get("/api/layouts/everything?client=c2&device=desktop").get_json()
    assert seeded["tabs"] == own["tabs"]
    assert (
        client.get("/api/layouts/everything?client=c2&device=mobile").get_json()["tabs"]
        == {}
    )
...
def test_clients_and_the_inventory_document_are_served(
    client: FlaskClient, app: Flask
) -> None:
    shell = _shell(app)
    client.post("/api/projects", json={"name": "Alpha", "color": "#111111", "glyph": 1})
    ...
    assert (
        document["clients"][0]["active_view"] == "alpha"
        and document["clients"][0]["is_connected"] is True
    )


def test_a_recorded_client_reads_the_seed_of_its_own_device_kind(
    client: FlaskClient, app: Flask
) -> None:
    ...
    mobile_body = {
        ...
        "dockview": {"panels": {"p0": {}}},
        "tabs": {
            "p0": {"address": str(_FILES), "tab_id": str(_TAB), "last_focused_ms": 0}
        },
    }
    ...
    seeded = client.get("/api/layouts/everything?client=c2&device=desktop").get_json()
    assert seeded["device_kind"] == "mobile" and seeded["tabs"]["p0"]["address"] == str(
        _FILES
    )
...
def test_the_broadcast_endpoint_validates_its_input(client: FlaskClient) -> None:
    ...
    assert (
        client.post(
            "/api/layout/broadcast", json={"op": "context"}, environ_base=_NOT_LOOPBACK
        ).status_code
        == 403
    )
    ...
    assert (
        client.post(
            "/api/layout/broadcast", json={"op": "open", "args": []}
        ).status_code
        == 400
    )
    assert (
        client.post(
            "/api/layout/broadcast", data="{", content_type="application/json"
        ).status_code
        == 400
    )


def test_the_read_ops_answer_from_the_state_files_and_the_activity_log(
    client: FlaskClient, app: Flask
) -> None:
    shell = _shell(app)
    client.post("/api/projects", json={"name": "Alpha", "color": "#111111", "glyph": 1})
...
def test_an_op_is_attributed_to_the_client_that_last_messaged_the_requesting_agent(
    ...
    shell.layouts.save_browser_layout(
        "alpha", "c7", layout_showing(_TERMINAL_1), None, TEST_NOW
    )

    assert (
        _broadcast(client, "inspect", {"view": "alpha"}).get_json()["client_id"] is None
    )
    ...
    assert [panel["address"] for panel in attributed["layout"]["panels"]] == [
        str(_TERMINAL_1)
    ]
    assert (
        _broadcast(client, "inspect", {"view": "alpha"}, agent_id="agent-2").get_json()[
            "client_id"
        ]
        is None
    )
    assert (
        _broadcast(client, "inspect", {"view": "alpha", "client": "c1"}).get_json()[
            "client_id"
        ]
        == "c1"
    )
    ...
    assert (
        _broadcast(client, "inspect", {"view": "alpha", "client": "../c1"}).status_code
        == 400
    )


def test_load_switches_the_requesting_agents_client(
    client: FlaskClient, app: Flask
) -> None:
    client.post("/api/projects", json={"name": "Alpha", "color": "#111111", "glyph": 1})
    _shell(app).activity.append_message(
        "c7", "desktop", "everything", "chat", "agent-1", "hello"
    )
    ...
    assert drain_messages(client_queue) == [
        {"type": "active_view_changed", "client_id": "c7", "view_id": "alpha"}
    ]
    ...
    assert (
        _broadcast(client, "load", {"view": "alpha"}, agent_id="agent-2").status_code
        == 412
    )
    assert (
        _broadcast(
            client, "load", {"view": "alpha", "client": "nobody"}, agent_id="agent-2"
        ).status_code
        == 404
    )
    explicit = _broadcast(
        client, "load", {"view": "alpha", "client": "c8"}, agent_id="agent-2"
    )
    assert (
        explicit.status_code == 200 and explicit.get_json()["target_client_id"] == "c8"
    )


def test_document_ops_edit_the_target_clients_file_and_announce_the_write(
    client: FlaskClient, app: Flask
) -> None:
    shell = _shell(app)
    client.post("/api/projects", json={"name": "Alpha", "color": "#111111", "glyph": 1})
    ...
    assert (
        _broadcast(client, "open", {"address": "terminal:terminal-1"}).status_code
        == 400
    )
    assert _broadcast(client, "open", {"address": "app:nope"}).status_code == 404
    assert (
        _broadcast(
            client, "open", {"address": "app:terminal?instance=terminal-9"}
        ).status_code
        == 404
    )
    ...
    assert stored is not None and [
        str(tab.address) for tab in stored.tabs.values()
    ] == [str(_TERMINAL_1)]
    assert (
        stored.dockview is not None
        and stored.dockview["grid"]["root"]["type"] == "branch"
    )
    ...
    assert (
        len(updates) == 1
        and updates[0]["client_id"] == "c1"
        and updates[0]["view_id"] == "alpha"
    )
    ...
    assert _panel_addresses(
        _broadcast(client, "open", {"address": str(_TERMINAL_1)}).get_json()["layout"]
    ) == [str(_TERMINAL_1)]
    ...
    assert [
        leaf["panels"][0]["address"] for leaf in tree["children"][0]["children"]
    ] == [
        str(_TERMINAL_1),
        str(_FILES),
    ]
    bad_anchor = _broadcast(
        client, "split", {"address": str(_TERMINAL_2), "relative_to": "app:nope"}
    )
    assert (
        bad_anchor.status_code == 404 and "app:nope" in bad_anchor.get_json()["detail"]
    )
    assert (
        _broadcast(
            client, "split", {"address": str(_FILES), "direction": "sideways"}
        ).status_code
        == 400
    )
    ...
    assert (
        _broadcast(client, "focus", {"address": "app:browser?instance=x"}).status_code
        == 404
    )
    ...
    assert [
        panel["address"]
        for panel in moved.get_json()["layout"]["tree"]["children"][0]["panels"]
    ] == [
        str(_TERMINAL_1),
        str(_FILES),
    ]
    ...
    assert closed.status_code == 200 and _panel_addresses(
        closed.get_json()["layout"]
    ) == [str(_TERMINAL_1)]
...
def test_an_op_lands_with_no_browser_connected_and_never_on_a_guessed_client(
    client: FlaskClient, app: Flask
) -> None:
    shell = _shell(app)
    client.post("/api/projects", json={"name": "Alpha", "color": "#111111", "glyph": 1})
    ...
    assert landed.status_code == 200 and _panel_addresses(
        landed.get_json()["layout"]
    ) == [str(_TERMINAL_1)]
    assert shell.layouts.read_client_layout("alpha", "c1") is not None
    assert (
        _broadcast(
            client, "open", {"address": str(_TERMINAL_1), "client": "nobody"}
        ).status_code
        == 404
    )
    ...
    shell.activity.append_message(
        "c3", "desktop", "everything", "chat", "agent-2", "hello"
    )
    attributed = _broadcast(
        client, "open", {"address": str(_FILES)}, agent_id="agent-2"
    )
...
def test_view_edits_that_views_file_and_switches_the_client_to_it(
    client: FlaskClient, app: Flask
) -> None:
    shell = _shell(app)
    client.post("/api/projects", json={"name": "Alpha", "color": "#111111", "glyph": 1})
    ...
    assert (
        _broadcast(
            client, "open", {"address": str(_TERMINAL_1), "view": "Nowhere"}
        ).status_code
        == 404
    )
    ...
    shell.layouts.save_browser_layout(
        "alpha", "seed-maker", layout_showing(_FILES), None, TEST_NOW
    )
    _record_client(app, "c2", "everything")
    inherited = _broadcast(
        client, "open", {"address": str(_TERMINAL_1), "view": "alpha", "client": "c2"}
    )
...
def test_open_of_a_bare_app_creates_through_the_relay_inside_the_op(
    ...
    inventory = build_inventory(
        write_registry(
            tmp_path / "apps.toml",
            registry_row_toml(
                "stub", stub_app_url, True, actions=[("new", "New"), ("other", "Other")]
            ),
        ),
        broadcaster,
        fetcher=HttpInstanceFetcher(),
    ...
    assert _panel_addresses(created.get_json()["layout"]) == [
        "app:stub?instance=stub-1"
    ]
    assert [
        (str(record.key), record.title, str(record.url))
        for record in stub_source.records
    ] == [("stub-1", "Stub 1", "/x")]
    assert "create:new:{'path': '/x'}" in stub_source.calls
    # An action the app does not declare is the app's own 400, passed through.
    assert (
        _broadcast(
            client, "open", {"address": "app:stub", "action": "other"}
        ).status_code
        == 400
    )
    ...
    assert (
        ...
        and split.get_json()["created_address"] == "app:stub?instance=stub-2"
    )
    # The app's refusal reaches the caller as the op's error, and nothing is docked.
    assert (
        _broadcast(
            client, "open", {"address": "app:stub", "action": "nope"}
        ).status_code
        == 400
    )
...
def test_transient_ops_reach_the_target_clients_windows(
    client: FlaskClient, app: Flask
) -> None:
    client.post("/api/projects", json={"name": "Alpha", "color": "#111111", "glyph": 1})
    ...
    _shell(app).activity.append_message(
        "c1", "desktop", "alpha", "chat", "agent-1", "hello"
    )

    assert (
        _broadcast(client, "maximize", {"address": "terminal:terminal-1"}).status_code
        == 400
    )
    ...
    assert (
        maximized.status_code == 200
        and maximized.get_json()["target_client_id"] == "c1"
    )
    ...
    refreshed = _broadcast(
        client, "refresh", {"address": str(_FILES)}, agent_id="agent-9"
    )
    assert (
        refreshed.status_code == 200
        and refreshed.get_json()["target_client_id"] is None
    )
    ...
    assert (
        _broadcast(
            client, "refresh", {"address": str(_TERMINAL_1)}, agent_id="agent-9"
        ).status_code
        == 412
    )


def test_reload_system_interface_reaches_every_view_and_null_args_are_refused(
    client: FlaskClient, app: Flask
) -> None:
    everything_queue = _register_client(app, "c1", "everything")
    alpha_queue = _register_client(app, "c2", "alpha")
    response = client.post(
        "/api/layout/broadcast", json={"op": "reload_system_interface"}
    )
    ...
    assert (
        client.post(
            "/api/layout/broadcast", json={"op": "refresh", "args": None}
        ).status_code
        == 400
    )
