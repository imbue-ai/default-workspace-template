...
def test_idle_agent_with_a_stale_queue_is_swept_without_a_transition(
    ...
    try:
        ...
        with agent_manager._lock:
            seeded_state = agent_manager._agents["agent-1"]
            agent_manager._queued_messages_by_agent["agent-1"] = stale
            agent_manager._agents["agent-1"] = seeded_state.model_copy_update(
                to_update(seeded_state.field_ref().queued_messages, stale)
            )
