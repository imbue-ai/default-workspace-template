...
def _browsers_passthrough() -> Response:
    ...
    can no longer reach the daemon's ``/browsers`` endpoint on its own origin
    directly. This server-side hop forwards ``GET`` / ``POST /api/browsers``
    to the registered ``browser`` service's local backend and relays the
    backend's body and status verbatim (GET returns ``{"browsers": [...]}``;
    POST relays the daemon's create result, including its 400/409/503
    rejections). Returns a 503 JSON error when the service is not registered
    or unreachable.
    """
    state = get_state()
