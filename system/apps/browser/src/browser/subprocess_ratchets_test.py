...
#
# ``xclipboard.py`` moves clipboard payloads, which are bytes (an image, not text) and
# arrive on the child's stdin. Neither entry point in the library expresses binary IO or
# stdin, and xclip talks to the X server rather than a terminal.
#
# ``session.py`` starts the SHARED pulseaudio daemon, which is deliberately meant to
# outlive any one browser and be adopted by the next through its ``pactl info`` probe.
# Its only reaper today is the group kill on a stop that overruns stopwaitsecs, so
# detaching it would leave it running for good. Waiving the whole file is broader than
# that one spawn -- every other spawn in it is already detached, and a new one here would
# go unnoticed.
_ALLOWED_RAW_SPAWN_FILES = ("xclipboard.py", "session.py", *TEST_FILE_PATTERNS)
