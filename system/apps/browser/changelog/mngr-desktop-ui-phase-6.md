...
- The shell's close hint now marks the browser the closed window showed as `window_seen` before the sweep it brings, so a browser whose window opened and closed between two sweeps is stopped like any other; the hint route's test waits for the sweep it schedules instead of racing it.
