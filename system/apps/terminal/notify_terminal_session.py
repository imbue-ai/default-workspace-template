...
# posters do (system/scripts/layout.py, the update-self skill's
# scripts/update_self.py): honor the MINDS_WORKSPACE_SERVER_URL override and
# default to the server's default bind address (config.system_interface_host is
# 127.0.0.1). Using 127.0.0.1 rather than "localhost" avoids an IPv6 (::1)
# resolution that the IPv4-only server would refuse.
