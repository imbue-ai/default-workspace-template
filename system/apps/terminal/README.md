...
`run_ttyd.sh` registers the app's manifest (`app.toml`, with the icon it
names) and the fixed port (7681) via `system/scripts/forward_port.py`, writes
the discovery event, installs the
