...
def replace_program_block(config_text: str, program: str, replacement_block: str) -> str | None:
    ...
    if span is None:
        return None
    start_idx, end_idx = span
    # A section taken from the end of another file may have no closing newline, which would
    # otherwise run the next section's header onto the last line spliced in.
