...
One limit, which the sign-in screen states: a chat already running keeps the key it was created with. Re-keying an account reaches chats created afterwards, so a chat on a key that has been replaced has to be started again.
