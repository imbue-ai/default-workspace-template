...
With one chat to send to, the `send` launch path sends the text there at once and selects it, with no picker; with no chat at all it starts a new chat with the text.
