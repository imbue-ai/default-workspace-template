...
export interface HarnessCatalog {
  ...
  // payload from the backend HarnessSpec. Always present, like `label`: the page is
  // served by the same process as the route, so there is no older backend to omit it.
  popups: HarnessPopup[];
}
