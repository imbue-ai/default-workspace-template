...
from imbue.chat.element_references import ElementReferenceError
from imbue.chat.element_references import get_element_references_directory
from imbue.chat.element_references import validate_reference_envelope
from imbue.chat.element_references import write_element_reference_file


def _envelope() -> dict[str, object]:
    return {"element_reference": {"app": "chat", "tag": "a", "text": "Read the intro", "ancestors": []}}
...
def test_a_reference_is_written_whole_to_a_fresh_private_file(tmp_path: Path) -> None:
    ...
    assert json.loads(written.read_text()) == _envelope()
...
def test_anything_but_one_object_under_the_key_is_refused(reference: dict[str, object]) -> None:
    with pytest.raises(ElementReferenceError):
        validate_reference_envelope(reference)
