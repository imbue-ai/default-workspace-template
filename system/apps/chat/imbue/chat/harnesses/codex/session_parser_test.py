...
def test_user_bubble_id_is_stable_across_rereads() -> None:
    ...
    the watcher dedups it -- not a duplicate bubble. This is why the id is content-derived,
    never derived from the line's physical position."""
    line = _user_line("hello codex")
    first = parse_lines(line, {})
    reread = parse_lines(line, {})
    assert first == reread
...
def test_user_bubble_id_differs_for_distinct_sends() -> None:
    ...
    a = parse_lines(_user_line("yes"), {})[0]["event_id"]
    b = parse_lines(_user_line("no"), {})[0]["event_id"]
    c = parse_lines(_user_line("yes", timestamp="2026-07-19T10:00:05.000Z"), {})[0]["event_id"]
    # different text
    assert a != b
...
def test_new_schema_item_completed_user_message_renders() -> None:
    ...
    events = parse_lines(_item_user_line("how we doin'"), {})
    assert len(events) == 1
...
def test_empty_item_user_message_is_skipped() -> None:
    assert parse_lines(_item_user_line(""), {}) == []
...
def test_non_user_item_completed_is_dropped() -> None:
    ...
    assert parse_lines(_item_line("AgentMessage"), {}) == []
    assert parse_lines(_item_line("CommandExecution"), {}) == []
    assert parse_lines(_item_line("Reasoning"), {}) == []
...
def test_task_markers_become_turn_lifecycle_events() -> None:
    ...
    started = parse_lines(
        {"timestamp": "2026-07-19T10:00:00Z", "type": "event_msg", "payload": {"type": "task_started"}}, {}
    )
    assert len(started) == 1
    assert started[0]["type"] == "special"
    assert started[0]["kind"] == "turn_started"
    completed = parse_lines(
        {"timestamp": "2026-07-19T10:00:02Z", "type": "event_msg", "payload": {"type": "task_complete"}}, {}
    )
...
def test_function_call_and_output_link_by_call_id() -> None:
    ...
    call = parse_lines(call_line, name_map)[0]
    out = parse_lines(out_line, name_map)[0]
...
def test_turn_markers_expose_turn_id_field() -> None:
    ...
    complete = {"timestamp": "t", "type": "event_msg", "payload": {"type": "task_complete", "turn_id": "tid1"}}
    aborted = {"timestamp": "t", "type": "event_msg", "payload": {"type": "turn_aborted", "turn_id": "tid1"}}
    assert parse_lines(started, {})[0]["turn_id"] == "tid1"
    assert parse_lines(complete, {})[0]["turn_id"] == "tid1"
    assert parse_lines(aborted, {})[0]["turn_id"] == "tid1"


def test_marker_without_turn_id_falls_back_to_a_content_derived_id() -> None:
    """A marker codex gave no ``turn_id`` gets an id from its own content, so a re-read of the
    same line dedups and two codex agents of one chat can never mint one id from one line
    number; a marker at another time is another event."""
    started = {"timestamp": "t", "type": "event_msg", "payload": {"type": "task_started"}}
    event = parse_lines(started, {})[0]
    assert event["event_id"].startswith("codex-task_started-t-")
    assert parse_lines(started, {})[0]["event_id"] == event["event_id"]
    later = {"timestamp": "t2", "type": "event_msg", "payload": {"type": "task_started"}}
    assert parse_lines(later, {})[0]["event_id"] != event["event_id"]
...
def test_failed_script_output_sets_is_error() -> None:
    ...
    assert parse_lines(failed, name_map)[0]["is_error"] is True
    assert parse_lines(ok, name_map)[0]["is_error"] is False
...
def test_detail_reconstructs_the_full_input_and_output() -> None:
    ...
    tc = parse_lines(call, {})[0]["tool_calls"][0]
    assert "input_preview" not in tc
    detail = parse_line_detail(call)["codex-call-c1"]
    ...
    detail = parse_line_detail(result)["codex-result-c1"]
...
def test_reasoning_line_marks_the_next_assistant_event_thinking_source() -> None:
    ...
    assert parse_lines(readable, {}) == [{"type": THINKING_SOURCE_MARKER_TYPE, "readable": True}]
    ...
    assert parse_lines(unreadable, {}) == [{"type": THINKING_SOURCE_MARKER_TYPE, "readable": False}]
    assert parse_reasoning_detail(unreadable) is None


def test_non_conversation_lines_are_dropped() -> None:
    assert parse_lines({"timestamp": "t", "type": "session_meta", "payload": {"type": "x"}}, {}) == []
    assert parse_lines({"timestamp": "t", "type": "turn_context", "payload": {}}, {}) == []
    non_dict_payload: dict[str, Any] = {"type": "event_msg", "payload": "not-a-dict"}
    assert parse_lines(non_dict_payload, {}) == []
...
def test_turn_context_effective_model_stamps_assistant_messages() -> None:
    ...
    assert parse_lines(fallback, {}, turn_state) == []
    later = parse_lines(_assistant_line("m3", "z"), {}, turn_state)
...
def test_a_failed_turn_surfaces_its_reason_as_a_message() -> None:
    ...
    """
    events = parse_lines(_task_complete({"message": _CODEX_401}), {})
...
def test_a_failed_turn_classifies_off_the_structured_tag() -> None:
    ...
    events = parse_lines(
        _task_complete({"message": "the model is busy", "codex_error_info": {"type": "server_error"}}), {}
    )
    failure = events[0]
