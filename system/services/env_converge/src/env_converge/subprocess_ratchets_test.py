...
env-converge runs apt, npm and the env.d unit scripts from a background process group on
the workspace's tmux terminal -- and a package manager drawing progress is exactly the
kind of child that touches one. See ``detached_subprocess.runner`` for the mechanism.
