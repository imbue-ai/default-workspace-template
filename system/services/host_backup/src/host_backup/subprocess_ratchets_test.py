...
host-backup runs restic and btrfs from a background process group on the workspace's tmux
terminal, so a child that inherits that terminal can stop the whole service when it is
killed -- see ``detached_subprocess.runner`` for the mechanism.
