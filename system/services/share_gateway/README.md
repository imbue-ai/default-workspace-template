...
1. **The gateway HTTP service** (Flask, `127.0.0.1:8791`): caddy's
   ...
   two copies of one signed value: plain `imbue_machine_session` for top-level
   visits, and `imbue_machine_session_partitioned` with the CHIPS `Partitioned`
   attribute for the hosted chrome's cross-site iframe -- either copy opens the
   session), re-reads `data/.secrets/share_grants.toml` on every request
