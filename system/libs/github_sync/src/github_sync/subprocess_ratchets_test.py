"""Ratchet confining this service's subprocess spawning to the detached runner.

github-sync is opt-in, but when it is on it is a supervisord program like the rest: its tick
loop shells out to git and to the latchkey CLI from a background process group on the
workspace's tmux terminal. See ``detached_subprocess.runner`` for what that costs.
