...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A settings_overrides leaf on a plugin-registered agent type, in every -S
        # spelling -- the shape a per-create Claude setting takes.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # A settings_overrides leaf on a custom agent type. mngr parses a -S as its
        # own config layer, so the type resolves without the settings file's
        # parent_type and validates against the base agent config, which has no
        # settings_overrides field. This is why a per-create Claude setting has to
        # target a plugin-registered type such as `claude`.
