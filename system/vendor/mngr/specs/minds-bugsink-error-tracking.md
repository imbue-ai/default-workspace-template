...
The application-level claims in this spec were verified by a working prototype of an earlier Modal-hosted revision (against a Neon Postgres database): migrations and steady-state operation on Postgres, eager-mode digestion, retention behavior, concurrency safety, SDK compatibility with our pinned `sentry-sdk` (2.59.0 at the time; since bumped, see below), API-driven provisioning, and failure modes (dead backend, process kill, redeploy). The VPS-hosting specifics (ingest path shapes through the caddy gate, tunneled-UI behavior) are pinned best-effort in code and confirmed by the dev bring-up's validation gates (see `apps/minds/docs/deploy/bugsink-bringup.md`).
...
Bugsink bring-up is an explicit, earlier step per tier; in the steady state there is no ordering constraint. The operator runbook is `apps/minds/docs/deploy/bugsink-bringup.md`; the shape:
