...
* `populate_context_post_run` fills `AgentContext`: the *workspace agent's* token counts (the decider's go under `metadata.decider_usage`, since harbor's own fields would report the simulated user's tokens as the agent's), and `metadata` with descriptive stats (turns completed, avg words per agent turn, timed_out). Harbor's cost field is left unset -- the driver prices nothing, and `check-run` prices the tokens.
