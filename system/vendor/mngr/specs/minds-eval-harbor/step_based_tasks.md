...
- Harbor sums one `AgentContext` per step into the trial's totals while the usage account the driver resolves is the whole conversation's, so each step publishes only the tokens consumed since the previous step's publish. An output-token figure the stream never reported publishes nothing rather than a partial one, and leaves what earlier steps published standing. Harbor's cost field is left unset on every step: the driver records tokens per model and prices nothing, and `minds-evals check-run` prices them when it builds a report.
