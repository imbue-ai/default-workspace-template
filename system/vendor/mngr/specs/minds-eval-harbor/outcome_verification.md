...
That capture is cheap and lands in Phase 1: at evidence-collection time the driver records the delivered repo's committed state as `verification/deliverable.bundle`, an incremental `git bundle <base>..HEAD` holding only the commits made in the workspace (the template's first-boot commit, then the agent's), plus the repo's `HEAD` sha and `git status --porcelain` output.
