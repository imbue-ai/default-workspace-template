...
- Tags cut before that pin (every one through `minds-v0.6.2`) float on the bare tag, so `template_base_image.py` also records, per snapshot timestamp, the digest-pinned base whose packages that snapshot covers (`PINNED_BASE_IMAGE_REF_BY_APT_SNAPSHOT_TIMESTAMP`). The pool bake's seed of such a tag (`minds-admin pool create --from-tag`, and the gen-2 migration's image seed) passes it to `docker build` as a `--build-context` override of the floating `FROM` (imbue-ai/mngr-internal#1143); the release tests below check every entry is still pullable and still installs the toolchain from its snapshot. Add an entry when a new snapshot is cut while floating tags still need baking; the table goes away with the last such tag.
