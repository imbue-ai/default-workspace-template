...
- **The SuperTokens core is a dependency of every session check**: the SDK reaches it for `check_database` verification, for the first access token minted after a refresh (even on read routes), for the browser-session cookie, and for the live email lookup. When the core answers 5xx or is unreachable, the connector retries the call once and then answers a retryable 503 `{"code": "auth_upstream_unavailable", ...}` (counted by the `supertokens_core_unavailable` metric) instead of a 500 -- a core blip is not a bug in the connector. A core 4xx still takes the unexpected-exception path.
...
Every stop of a workspace stamps `pool_hosts.stop_kind` with why it happened, and every start clears it (`specs/workspace-stop-kinds.md`): `owner` (the user's own stop, from any of their devices), `maintenance` (an operator hold, such as the gen-1 to gen-2 migration), `idle` (an operator stop to free capacity, such as `server drain`), `suspension` (the suspend fan-out; the unsuspend fan-out rewrites those rows to `idle`) or `retired` (a workspace that can never run again: its version cannot run on the gen-2 fleet and its data was archived for its owner -- `minds-admin workspaces retire`). NULL is a stop recorded before the column existed and reads as `owner`. `GET /workspaces` and `GET /workspaces/{id}` carry the kind as `stop_kind`. The owner's `POST /workspaces/{id}/start` refuses `maintenance` and `suspension` rows from `stopping` onwards with `409 {"code": "workspace_under_maintenance", "message": "This machine is undergoing maintenance and will be back shortly."}` (the same detail the cutover's parked-row guard answers), and a `retired` row with `409 {"code": "workspace_retired", "message": "This machine has been retired and cannot be started again. ..."}`; the operator start ignores a `maintenance` / `suspension` kind, which is how a held workspace comes back, but refuses a `retired` row the same way (re-kind it with `set-stop-kind idle` to start one deliberately).
...
- `POST /admin/workspaces/{host_db_id}/start` -- Operator start of one `stopped` workspace (the owner start transition without the ownership and quota checks; the gen-2 cutover runbook's pre-window step). A `retired` row is refused (409 `workspace_retired`). Exposed as `minds-admin workspaces start`.
