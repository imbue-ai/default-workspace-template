...
- `POST /hosts/{host_db_id}/enable-sharing` -- The web chrome's server-side bring-up for one of the caller's leased hosts: creates/rotates the share record, then writes the share materials into the container over SSH pinned to the row's recorded container host key. A workspace the minds desktop app adopted serves a rotated host key the connector never learns, so the write fails the strict host-key check and the route answers `409 {"code": "workspace_managed_by_desktop", ...}`: enable sharing from the desktop app instead.
