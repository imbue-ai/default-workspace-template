...
* ``stopping`` -- VM halted, upload in flight, still restartable in place
* ``stopped``  -- artifact in object storage, slot freed
...
spawn a supervisor, and return 202; clients poll ``GET /workspaces/{id}``.
"""
...
class WorkspaceInfo(BaseModel):
    ...
    vps_address: str | None = Field(description="Box address (None while stopped)")
    ssh_port: int | None = Field(description="VM-root forwarded port (None while stopped)")
    ssh_user: str = Field(description="SSH user on the VM")
    container_ssh_port: int | None = Field(description="Container forwarded port (None while stopped)")
...
def stop_workspace(request: Request, host_db_id: UUID) -> dict[str, object]:
    """Begin stopping a running workspace: halt its VM, upload it, free the slot.

    Asynchronous: CAS ``leased -> stopping``, spawn the transition
    ...
    with handle_endpoint_errors():
        ...
        try:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    "transition_error = NULL, transition_heartbeat_at = NULL "
        ...
        stop_start.spawn_supervisor(str(host_db_id))
...
def start_workspace(request: Request, host_db_id: UUID) -> dict[str, object]:
    """Begin starting a stopped (or still-stopping) workspace.

    Asynchronous: CAS into ``starting``, spawn the supervisor (which
    restarts in place when the VM is still on its origin box, or restores
    from the artifact otherwise), and return immediately. A start from
    ``stopped`` re-occupies a running-workspace slot, so it checks
    ``max_remote_workspaces`` under the same per-user lock the lease path
    uses; a restart of a still-``stopping`` row is not a new running
    workspace (stopping already counts as running) and skips the check.
    """
    with handle_endpoint_errors():
        ...
        try:
            ...
            if current_db_status not in ("stopped", "stopping"):
                raise HTTPException(
                    status_code=409,
                    detail=f"Workspace is {_WIRE_STATUS_BY_DB_STATUS.get(current_db_status, current_db_status)}"
            ...
            with conn:
                with conn.cursor() as cur:
                    ...
                    cur.execute(
                        ...
                        "WHERE id = %s AND status IN ('stopped', 'stopping')",
                        (str(host_db_id),),
                    )
                    updated = cur.rowcount
        finally:
...
def abandon_workspace(request: Request, host_db_id: UUID, body: AbandonWorkspaceRequest) -> dict[str, object]:
    ...
    with handle_endpoint_errors():
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "UPDATE pool_hosts SET status = 'crashed', transition_error = %s "
                    "WHERE id = %s AND status IN ('leased', 'stopping', 'stopped', 'starting')",
                    (body.reason[:2000], str(host_db_id)),
                )
                updated = cur.rowcount
            conn.commit()
        finally:
