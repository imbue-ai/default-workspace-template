...
collector installs (box prep consumes the rendered script via
``--extra-prep-script``; relays are installed directly over SSH).
...
def provision(tier: str, ordinal: int, ovh_region: str, flavor: str, image: str, ssh_public_key_file: Path) -> None:
    ...
    logger.info("Observability instance ready: name={} id={} ip={}", instance_name, created["id"], ip_address)
...
def dns(hostname: str, ip: str) -> None:
    ...
    only Cloudflare's ranges, so the proxy is the sole way in.
    """
    is_changed = reconcile_ingest_dns_record(
        ...
        hostname=TelemetryHostname(hostname),
        ip=ip,
    )
...
def render_collector_install(role: str, tier: str, ingest_url: str, credential_env_var: str, out: Path) -> None:
    """Render the self-contained collector install script (box prep consumes it via --extra-prep-script)."""
...
def install_collector(
    ...
    by the prep flow, which consumes the rendered script via
    ``minds server prep --extra-prep-script`` instead.
    """
    config = _collector_config_from_options(role, tier, ingest_url, credential_env_var)
