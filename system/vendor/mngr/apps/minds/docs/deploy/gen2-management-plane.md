...
3. **The public address** — correct for every box without a live lockdown (including a first-ever prep). Once a prep reports that it installed the lockdown, `server setup` / `server prep` refuse this rung for their remaining round trips and fail instead of marking an unreachable box `ready` (the row's status is left as it was, `installing` for a first `setup`; fix the WireGuard path and re-run).
...
`sync-peers` re-renders each prepped gen-2 box's `wg0.conf` from the committed list. Where only the `[Peer]` blocks changed, the box applies them live with `wg syncconf` (the interface stays up, the operator's own session through it survives, and the change takes effect in milliseconds); an unchanged box is left alone; only an `[Interface]` change (address, port, key) restarts `wg-quick@wg0`, which severs the syncing operator's tunnel session and costs tens of seconds of reconnect per box. On each box the rendered file is first parsed by `wg` itself on a scratch interface, and a config `wg` rejects fails the box before the installed `wg0.conf` is touched; if the restart still fails, the previous config is restored and brought back up. It syncs one **canary** box first (the first prepped box in the registry, or `--canary-server-id <id>`), then re-dials it over a fresh session and reads `wg0` back: unless the interface is active with exactly the committed peers, the command stops there and touches no other box, so a peer list the boxes reject can take one box's overlay down but never the fleet's (that happened once, on 2026-09-21, with an operator key `wg` refused). Only after the canary verifies are the remaining boxes synced, in parallel (`--max-concurrency` bounds it). It resolves each box's dial automatically (the userspace tunnel, else a reachable overlay route, else the public address — see Operator transport above), so a post-lockdown sync just works. The box you are adding *yourself* to is the exception: your tunnel cannot reach it until its peers include you and its locked public `:22` drops you, so another allowed path (an existing operator, or the bounce host under Break-glass) must run that sync. A restart (an `[Interface]` change) can drop the syncing operator's own tunnel session mid-run and report that box as failed; re-running converges (the config is already in place, so the second pass changes nothing).
...
- The rendered `wg0.conf` is validated on-box before it is installed: a copy (`ListenPort` dropped, since the live interface holds the port) goes through `wg-quick strip` and `wg setconf` on a throwaway `wgchk<pid>` interface, so anything `wg` refuses (a malformed peer key, a bad `AllowedIPs`) fails the prep or sync with the live interface untouched. A config that passes is then applied with `wg syncconf wg0 <(wg-quick strip wg0)` when only its `[Peer]` blocks differ from the installed one and the interface is up, else by restarting `wg-quick@wg0`; if the reload or restart fails, the previous `wg0.conf` (kept as `wg0.conf.mngr-previous` for the duration) is restored and re-applied.
