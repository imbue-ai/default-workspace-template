    ...
    * No ``mngr message`` has been sent.
    ...
    def build_request_detail_payload(
        ...
        if service_info is None:
            return UiUnknownScopeDetail(request_id=req_event.request_id, scope=payload.scope)

        parsed_id = AgentId(req_event.agent_id)
        ws_name = resolve_workspace_display_name(backend_resolver, parsed_id, fallback=req_event.agent_id)
    ...
    def apply_grant_request(
        ...
        maybe_recover_host_permissions(self.latchkey, get_state().backend_resolver, req_event)
        service_info = self.services_catalog.get_by_scope(payload.scope)
        if service_info is None:
        ...
        request_event_id = req_event.request_id
