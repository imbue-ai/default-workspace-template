...
- `minds-admin wireguard {config, sync-peers, install-onetun}` -- the gen-2 management WireGuard overlay (operator client configs; fleet peer sync from the `[management_plane]` table of the tier's committed `deploy.toml`; the pinned onetun install from the artifact mirror). `config` and `sync-peers` address a tier, not an env: `--tier <dev|ci|staging|production>` selects the committed operator list, the SSH CA, and the tier's box registry (its `secrets/minds/<tier>/neon/DATABASE_URL` pool DB), so no activation is needed; without the flag the activated env's tier is used.
...
- `minds-admin workspaces {stop, start, retire, set-stop-kind, abandon, release}` -- workspace-lifecycle escape hatches (operator force-stop with a required `--kind maintenance|idle|suspension|retired`, which decides whether the owner may start it again; operator start of a stopped workspace, which clears the kind; `retire` stops a workspace for good -- the kind for one the gen-2 migration cannot take, after `archives create` -- which nobody starts again; `set-stop-kind` to hand a held workspace back (`idle`; a row the cutover has parked stays refused by the connector's parked-row guard and needs `cutover rollback`) or hold an owner-stopped one (`maintenance`); mark-crashed; release a confirmed-abandoned lease through the connector's own destroy chain, any lifecycle status). See `specs/workspace-stop-kinds.md`.
- `minds-admin archives {candidates, create, list, links, download}` -- archive the gen-1 workspaces the migration cannot take (below the `minds-v0.3.10` floor, or without a release tag) into the tier's workspace-storage bucket as one zip each (the workspace's volume from a read-only btrfs snapshot plus its container's writable layer, streamed from the VM through the box's loopback SSH, nothing on the workspace changed), and hand their owners 7-day download links from a support document (`~/.minds-<env>/archives/reports/`). Runbook: the "Retiring the workspaces the migrate cannot take" section of `apps/minds/docs/deploy/gen2-cutover.md`.
