#!/bin/bash
# Run the rewardkit verifier (gates + quality dimensions), then compose the
# final gated reward. rewardkit owns all judging and scoring; finalize.py
# combines the two dimension scores (reward = quality if every gate passed,
# else 0.0) and distinguishes a graded 0.0 from a grading-infrastructure
# failure (judge API error / no parseable reward file), leaving no reward
# file in the latter case so harbor errors the trial instead of scoring it 0.
set -euo pipefail
