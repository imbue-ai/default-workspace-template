...
- `manifest.json` -- the evidence bundle's index. Every entry is one recorded probe with a `status`:
  ...
  Each entry also carries a `reason` (why it is not passing) and a `detail` (the concrete observation: how many apps were registered, what a probe answered, what a test command printed), and an optional `value` (a continuous measurement, such as the seconds a `timing` entry took).
  A `timing` entry is the one entry whose `passed`/`failed` is not a verdict on the delivery: it says whether there was a time to score at all. The seconds themselves are scored separately on a curve, so a `passed` timing entry naming a slow time is not a delivery failure, and a `failed` one says only that the client never declared its goal met or that another class it depended on fell short -- which those entries already report themselves. Grade the delivery, not the clock.
