...
- Before finishing, prune the comments on your branch's diff by running /crispy-comments: remove incidental history, defensive justification, correctness arguments, DRY restatement of volatile facts, commented-out code, and banners, keeping only comments that explain a non-obvious "why". Banners come out of every file the diff touches, not just the lines it added, and matching the surrounding file is never a reason to keep any of it. The /autofix pass also enforces this (the `comment_cruft` category in `.reviewer/code-issue-categories.md`), but running it yourself keeps the diff clean before review.
