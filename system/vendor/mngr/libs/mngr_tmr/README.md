...
Test map-reduce plugin for [mngr](https://github.com/imbue-ai/mngr). `mngr tmr`
collects tests via pytest and launches one agent per test; each test's
docstring is the contract for what it verifies. It polls agents to completion,
pulls successful work into local branches, integrates them with a reducer
agent, and generates an HTML report. (Behavior corpora are witnessed by
`mngr witness`, from `imbue-mngr-witness`, which runs on `imbue-mngr-mapreduce`
...
The canonical flag sets are the `tmr-mngr` / `tmr-minds`
