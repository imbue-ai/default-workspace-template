...
def test_stop_reads_effective_model_and_fast_from_transcript(tmp_path: Path) -> None:
    ...
    config_dir = tmp_path / "config"
    config_dir.mkdir()
    (config_dir / "settings.json").write_text(json.dumps({"model": "opus[1m]", "effortLevel": "max", "fastMode": True}))
...
def test_post_tool_use_reads_transcript_mid_turn(tmp_path: Path) -> None:
    ...
    config_dir = tmp_path / "config"
    config_dir.mkdir()
    (config_dir / "settings.json").write_text(json.dumps({"model": "opus[1m]", "effortLevel": "high", "fastMode": True}))
    transcript = tmp_path / "session.jsonl"
    transcript.write_text(
        json.dumps(
            {
