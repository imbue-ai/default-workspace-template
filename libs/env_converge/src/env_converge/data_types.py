...
class NpmGlobalState(FrozenModel):
    ...
    version_by_package: dict[str, str] = Field(description="Globally-installed npm packages and versions")
    recorded_at: datetime = Field(description="When this state was captured (UTC)")
...
class UvToolState(FrozenModel):
    ...
    version_by_tool: dict[str, str] = Field(description="uv-installed tools and versions")
    recorded_at: datetime = Field(description="When this state was captured (UTC)")
...
class UnitRunResult(FrozenModel):
    ...
    unit_name: str = Field(description="Script file name (e.g. 1000-playwright-fortress.sh)")
...
class OverlayApplyResult(FrozenModel):
    ...
    absolute_path: str = Field(description="The rootfs path that now symlinks into the overlay")
    is_adopted: bool = Field(description="Whether pre-existing rootfs content was moved into the overlay")


class ConvergeResult(FrozenModel):
    """Outcome of a converge pass (fast and/or slow phase)."""

    overlay_results: tuple[OverlayApplyResult, ...] = Field(description="Overlay entries applied")
    unit_results: tuple[UnitRunResult, ...] = Field(description="env.d units run, in order")
    installed_apt_packages: tuple[str, ...] = Field(description="Recorded apt packages installed this pass")
    installed_npm_packages: tuple[str, ...] = Field(description="Recorded npm globals installed this pass")
    installed_uv_tools: tuple[str, ...] = Field(description="Recorded uv tools installed this pass")
    unavailable_packages: tuple[str, ...] = Field(
        description="Recorded packages that could not be installed (package_unavailable events)"
    )
    is_fresh_rootfs: bool = Field(description="Whether this rootfs had no identity stamp (converge-first ordering)")
