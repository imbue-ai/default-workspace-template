...
def capture() -> None:
    ...
    workspace_dir = DEFAULT_WORKSPACE_DIR
    capture_all(default_record_dir(), read_pinned_snapshot_timestamp(workspace_dir), workspace_dir)
...
def upgrade() -> None:
    ...
    deltas = run_upgrade(default_record_dir(), DEFAULT_WORKSPACE_DIR, DEFAULT_OVERLAY_DIR)
...
def status() -> None:
    ...
    summary = {
        ...
        "recorded_snapshot_timestamp": base.snapshot_timestamp if base is not None else None,
        "committed_snapshot_timestamp": committed_timestamp,
        "is_upgrade_pending": base is not None and base.snapshot_timestamp != committed_timestamp,
        "recorded_manual_apt_count": len(apt_state.manual_packages) if apt_state is not None else None,
        "recorded_npm_global_count": len(npm_state.version_by_package) if npm_state is not None else None,
        "recorded_uv_tool_count": len(uv_state.version_by_tool) if uv_state is not None else None,
    }
