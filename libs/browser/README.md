...
- **Ownership** is one locked, compare-and-set state machine per browser. Agents
  acquire a browser to run a task and never preempt each other -- a second agent
  waits in a FIFO queue (monitor-and-wait). The human can take control from the
  UI at any time, which always wins, pins the browser to the human, and ends the
  agent's task. Ownership is bound to the live task connection, so a dead agent's
  browser is released automatically (no stuck locks).
- **CLI** (`agentic-browser-fleet`): the thin client agents use to drive the
  fleet -- `ls`, `new`, `task <id> "<prompt>"`, `lock`/`unlock`/`release`. A
  `task` hands a goal to a browser-use agent on that browser and streams its
  Thinking/Action trace to the calling agent's own output. See the
  `agentic-browser-fleet` skill.
