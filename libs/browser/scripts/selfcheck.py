...
What it reports:
  1. Environment & deps  -- Fortress/Xvfb/xclip present, deferred-install done, and
...
paint (it will sit on "Loading page…") until deferred-install finishes. Otherwise the
...
MARKER_DIR = "/var/lib/minds/deferred-install"
# The native libs pixelflux's wheel links at import time (see deferred_install.sh).
...
def check_env() -> None:
    ...
    _row("deferred-install", "ready" if ready else f"NOT ready -- {reason}")
    for pkg in ("fortress", "xvfb"):
        marker = os.path.join(MARKER_DIR, f"done.{pkg}")
        _row(f"  marker done.{pkg}", "yes" if os.path.exists(marker) else "NO (still installing / failed)")
