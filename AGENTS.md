...
You manage your work using `tk`, the vendored ticket tracker at `vendor/tk/`. It is the **only** task tracker available -- your harness's own built-in todo/plan tool (if it has one) should not be used instead. This is enforced differently per harness: opencode's `todowrite` tool is disabled directly in config; codex's `update_plan` has no config-level disable, so this is a deliberate instruction to avoid it, not an enforced block. `tk` stores two kinds of records, distinguished by the `--step` flag at creation:

- **Step records** (`tk create --step "..."`) are the replacement for your harness's built-in todo tool: turn-bound, creator-private progress markers that render as nodes on the user-facing chat progress view (a vertical timeline with a status icon and a one-line summary per step). Most turns use only these.
...
- **AGENTS.md**: (this file) update these instructions if you discover better ways to operate. Note: `CLAUDE.md` is a separate, claude-specific copy of most of this same content -- if a change applies to all harnesses, make it in both files.
...
`runtime/` is gitignored from the main branch (it includes `runtime/memory/` for shared agent memory and other transient state).
