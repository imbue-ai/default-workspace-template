...
- When adding a new app, use the `build-app-parallel` skill, which plans the build and runs it in parallel workers; the app ends up as a new package under `system/apps/` + a supervisord program entry + `forward_port.py` registration on its own port (`build-app` is the reference for those mechanics). Do NOT edit `system/apps/system_interface/` for this -- that's the top-level workspace UI, not a template for new apps.
