...
`/tmp` is memory, not disk: it is a tmpfs, so whatever you write there counts against the container's memory limit, and no process kill frees it. Keep it to small files. Anything that can run to gigabytes -- a copy of an app's data, a clone, a download, a backup export or restore -- goes on disk: a throwaway file under `/var/tmp` (outside the backup), and a copy of an app's data through the `update-app` skill's copy tools, which check that it fits.
